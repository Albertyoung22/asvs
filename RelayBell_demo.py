...
UDP Ë™ûÈü≥?•Êî∂Á´ØÔ??´Ê???UIÔΩú‰??ôÂÖ®?®Â??üËÉΩÔΩúÂ?Á´ØÈ??¢Áç®Á´ãÔ?‰∏çÂÖßÂª∫Âú®Á®ãÂ?Ë£°Ô?

- ‰øùÁ?ÔºöTkinter Ê°åÈù¢ UI?ÅUDP ?•Êî∂?ÅTTS?ÅMP3/YouTube ?≠Êîæ?Å‰??≥Ê??Æ„ÄÅËá™?ïÊ?Á®ã„ÄÅngrokÔºàÂèØ?∏Ô?

- Web ?ÖÊ?‰æ?APIÔº?translateÔºàmini_translator ÊµÅÁ?Ôºâ„Ä?send??sendmp3??cmd /state /files /upload /download /delete /timetable/* /schedules/* /logs* /piper/*

- ?ßÂª∫ Edge TTS ??PiperÔºàÁ¨¨‰∫åÈ?‰ΩçÔ???gTTS ??SAPI5

- UI ?áÂ??ÄÊ±Ç‰??¥Ô?‰∏ªÊéß?∞„ÄÅÊ?Á®ã„ÄÅÊ?Ê°?‰∏äÂÇ≥?ÅËá™Ë®Ç‰ªª??4 ?ÜÈ?ÔºàÂ?ËßÄÁ∂≠Ê?Ôº?



 ?∞Â??áË™ø?¥Ô?

1) Ëº∏ÂÖ• YouTube ???ÂæåÔ??≥Ë??É‰ª•?åÂΩ±?áÊ?È°?mp3?ç‰?Â≠òÂú® UPLOADSÔºàË?‰∏äÂÇ≥ MP3 ?åË??ôÂ§æÔºâÔ?‰∏¶Â?Ë©≤Ê??≠ÊîæÔºà‰??çÂà™?§Ô???

2) ?çÁ´ØÁ∂≤Â???ngrok URL ??QRCode ?πÁÇ∫ÂΩàÂá∫?åÁç®Á´ãÂ?Ë¶ñÁ??çÈ°ØÁ§∫Ô??´Ë?Ë£ΩË??ãÂ??âÈ?Ôºâ„Ä?
...
Â¶ÇÊ?Ë¶ÅÂ? EXEÔºöË?Á¢∫‰???mp3?Ålogo.ico?ÅÔ??ØÈÅ∏Ôºâffmpeg.exe ?æÂ??ÆÈ??ñÈÄèÈ? PyInstaller datas Â∏∂ÂÖ•??
...
# == [ANCHOR] ‰ΩøÁî®?ÖÂèØË™øÊï¥?ÄÂ°?==

# ===============================

USE_NGROK = False           # ?≥È???ngrok Â∞±Êîπ False

USE_EDGE_TTS = True         # ?ØÂê¶‰ΩøÁî® Edge TTS (??False ?áÁõ¥?•Ë∑≥?éÔ?‰ΩøÁî® Piper/gTTS)

USE_AI_GENERATION = True    # ?ØÂê¶?üÁî® AI Âª?í≠Á®øÁ???(??False ?á‰??ÄÂÆâË? Ollama)

HTTP_PORT = 5050            # ?πÁÇ∫ 5050 ‰ª•ÈÅø?ãË??≤Á?‰ΩîÁî®

PORT = 8888                 # UDP ??ÅΩ??

YT_AUTO_CLOSE_MIN = 30      # ?®Ëû¢ÂπïÈÄæÊ??™È??âÔ??ÜÈ?Ôºâ‚?‰øùÁ?Ê¨Ñ‰?

TIMETABLE_SCAN_SEC = 5      # ?íÁ??ÉÊ??±Ê?ÔºàÁ?Ôº?

DISABLE_UDP = False         # True ?Ç‰??üÂ? UDP ?•Êî∂Á∑íÔ?Á¥îÊú¨Ê©üÔ?

DISABLE_WEB = False         # True ?Ç‰??üÂ? Flask + ngrokÔºàÁ??¨Ê?Ôº?

# [NEW] Fire Alarm Hardware Config
FIRE_ALARM_PORT = None      # ?´ÁÅΩË®äË?Ëº∏ÂÖ•??(?•Ë®≠??None ?á‰??üÁî®)
FIRE_ALARM_PIN = "CTS"      # ?µÊ∏¨?≥‰?: CTS ??DSR
...
# ==========================================================
#  Âø´ÈÄüÊ?‰ª?-> ?¨Ê??≥Ê??†Â?Ë°?(?¥Êé•?≠Êîæ,‰∏çËµ∞ audio_proxy)
# ==========================================================
CMD_SOUND_TABLE = {
...
if getattr(sys, 'frozen', False):
    ...
    for p in candidates:
        ...
        if os.path.exists(p):
            ...
            # [CRITICAL] ?¥Â?Á¶ÅÁî® SSL È©óË?‰ª•‰øÆÂæ?Frozen App ‰∏≠Á? Edge TTS ????èÈ?
            try:
...
# ---- TkinterÔºàGUI ?∞Â??çÈ?Ë¶ÅÔ?----
# Render / Linux ?°È†≠Ê®°Â?‰∏?tkinter ‰∏ç‰?ÂÆöÂ??®Ô?‰ΩøÁî® try/except ‰øùË≠∑
_HAS_TKINTER = False
try:
    ...
    # --- tkinter StubÔºöË?ÂæåÈù¢?ÑÊ®°ÁµÑÂ±§Á¥öÁ?ÂºèÁ¢º‰∏çÊ? NameError ---
...
# ==== [schedules ?≤Â? JSON Ëß??Â∑•ÂÖ∑] ====

def _ensure_obj(value):

    """?äÂèØ?ΩÊòØ str ??JSON ?ÑÂ???dict/listÔºõÂÆπÂøçË¢´Â≠ó‰∏≤?ñÂÖ©Ê¨°„Ä?""
...
def _get_json_tolerant():
    ...
    """?ÄÂ§ßÂÆπÂøçÂ∫¶??request ËΩâÊ? Python ?©‰ª∂??""
...
try:
    ...
    HAS_DEEPTRANS = False
    print("[translate] deep_translator ?™Â?Ë£?)
...
def _get_server_location():
    ...
    res = {"lat": 25.0330, "lon": 121.5654, "city": "?∞Â?Â∏?, "region": "Taipei City"}
...
def _get_weather_report():
    ...
    loc_name = loc["city"] or loc["region"] or "?∞Â?Â∏?
    ...
    try:
        ...
        if r.status_code != 200: return "?°Ê??ñÂ?Ê∞?±°Ë≥áÊ?"
        ...
        if code in (1, 2, 3): status = "Â§öÈõ≤"

        elif code in (45, 48): status = "?âÈúß"

        elif 51 <= code <= 55: status = "ÊØõÊ???

        elif 56 <= code <= 57: status = "?çÈõ®"

        elif 61 <= code <= 65: status = "‰∏ãÈõ®"

        elif 66 <= code <= 67: status = "?∞Èõ®"

        elif 71 <= code <= 77: status = "‰∏ãÈõ™"

        elif 80 <= code <= 82: status = "??õ®"

        elif 85 <= code <= 86: status = "??õ™"

        elif code >= 95: status = "?∑Èõ®"
        ...
        try:
            ...
            if aq_r.status_code == 200:
                ...
                if aqi is not None:
                    ...
                    if aqi <= 50: q_status = "?ØÂ•Ω"

                    elif aqi <= 100: q_status = "?ÆÈÄ?

                    elif aqi <= 150: q_status = "Â∞çÊ??üÊ?Áæ§‰??•Â∫∑"

                    elif aqi <= 200: q_status = "‰∏çÂÅ•Â∫?

                    elif aqi <= 300: q_status = "?ûÂ∏∏‰∏çÂÅ•Â∫?

                    else: q_status = "?±ÂÆ≥"

                    aqi_str = f"ÔºåÁ©∫Ê∞??Ë≥™{q_status}ÔºåAQI?áÊï∏{aqi}"
        ...
        now_str = get_now().strftime("%HÈª?M??)

        return f"?æÂú®?ÇÈ? {now_str}ÔºåÊ??®‰?ÁΩ?{loc_name}ÔºåÁõÆ?çÊ∞£Ê∫?{temp} Â∫¶Ô??∏Â?ÊøïÂ∫¶ {humid}%ÔºåÂ§©Ê∞??Ê≥ÅÔ?{status}{aqi_str}??

    except Exception as e:

        print("Weather error:", e)

        return "Ê∞?±°Ë≥áÊ?ËÆÄ?ñÂ§±??



# ---- qrcodeÔºàËã•?°Â?‰ª•Ê?Á§∫‰ª£?øÔ?----
...
def api_ai_script():
    ...
    if not config.get("use_ai_generation", USE_AI_GENERATION):
        return jsonify(ok=False, error="AI ?üÊ??üËÉΩÂ∑≤Â???(Ë´ãËá≥Ë®≠Â??ÅÈù¢?ãÂ?)"), 403
    ...
    try:
        ...
        system_prompt = """
        ‰Ω†Áèæ?®ÊòØÂ∞àÊ•≠?ÑÊô∫?ßÂª£?≠Á≥ªÁµ?AI ?©Á???
        Ë´ãÊ†π?ö‰Ωø?®ËÄÖÊ?‰æõÁ??åÈ??µÂ??çÔ??üÊ?‰∏ÄÊÆµÁ? 50-100 Â≠óÔ?Ë™ûÊ∞£Ë¶™Â??ÅÊ≠£ÂºèÁ??≠Â†±Á®ø„Ä?
        
        ?êÈ?Ë¶ÅË?ÁØÑ„Ä?
        1. ?ÖËº∏?∫„ÄåÁ??áÂ??çÂÖßÂÆπÔ?ÁµïÂ?Á¶ÅÊ≠¢‰ΩøÁî®‰ªª‰?Ë°®Ê?Á¨¶Ë? (?å???ÅMarkdown ?ºÂ? (??*Á≤óÈ?**)?ÅÁ???(=) ?ñÂÖ∂‰ªñÁâπÊÆäÁ¨¶?ü„Ä?
        2. ?Ö‰??ôÊ?Ê∫ñÊ?ÈªûÁ¨¶??(Ôºå„ÄÇ„ÄÅÔ?ÔºÅÔ??å„Ä???
        3. ?®Ë?ÂøÖÈ?Ê∏ÖÊô∞?éÁ¢∫ÔºåÈÅø?ç‰Ωø?®ÂÆπ?ìÈÄ†Ê??¥Èü≥Â≠óË™§ËÆÄ?ÑË?ÂΩ?(‰æãÂ?ÔºöÈÅø?ç„ÄåË??çÁ?Â§öÈü≥Â≠óÔ??πÁî®?¥Ê?Á¢∫Á?Ë©???
        4. Ë™ûÂè•Ë¶ÅÈÄöÈ?ÊµÅÊö¢ÔºåÈÅ©?àË??≥Â???(TTS) ?óË???
        5. Ë´ã‰Ωø?®Á?È´î‰∏≠?áÔ?‰∏çË?‰ΩøÁî®?±Ê???
        """
        
        user_input = f"Âª?í≠?úÈçµÂ≠óÔ?{keyword}"
        ...
        try:
            ...
            if e.status_code == 404:
                ...
                return jsonify(ok=False, error="?ØË™§ÔºöÊâæ‰∏çÂà∞ AI Ê®°Â?ÔºåË??®Èõª?¶‰??∑Ë? `ollama pull gemma2:2b` ‰∏ãË?Ê®°Â???), 404
            raise e
        ...
        import re
        result_text = re.sub(r"^@\w+[:Ôºö]\s*", "", result_text)
...
def ws_web_handler(ws):
    """
    ?ÅÂ∏´/ÁÆ°Á??°Â?Á´ØÁî®??WebSocketÔºåÁî®‰æÜÊé•?∂Èü≥Ë®äÂª£??
    """
    with WEB_WS_LOCK:
...
def api_audio_proxy():
    """
    ËÆìÂ?Á´ØÂèØ‰ª•‰?Ëºâ‰ªª‰ΩïË∑ØÂæëÁ??≥Ë?Ê™îÔ??ÖÈ??≥Ë??ºÂ?Ôº?
    """
    path = request.args.get('path')
    if not path: return abort(400)
    
    # [Robustness] ‰øÆÊ≠£Ë∑ØÂ??ÜÈ?Á¨¶‰∏¶?øÂ?ÁµïÂ?Ë∑ØÂ??ªÊ?
    ...
    try:
        # ?ïÊ??≤Â? TAIGI_AUDIO_DIR ‰ª•Â?Â∞öÊú™ÂÆöÁæ©
    ...
    if path.startswith("uploads/"):
        ...
        if os.path.exists(cand):
            ...
            if not found:
                # ?ÄÂæåÂ?Ë©¶Á≥ªÁµ±Êö´Â≠?(‰ª?basename ?πÈ?)
...
def live_stream(ws):
    ...
    if is_broadcaster:
        ...
        text_area_insert(f"?î¥ ?¥Êí≠‰æÜÊ?Â∑≤ÈÄ?? ({request.remote_addr})", "Live")
        ...
        if _MPV and _HAS_PYGAME:
            ...
            try:
                ...
                text_area_insert(f"?¥Êí≠?≠Êîæ??MPV)?üÂ? (PID={proc.pid})", "Live")

            except Exception as e:

                text_area_insert(f"??MPV ?üÂ?Â§±Ê?: {e}", "Live")
             ...
             try:
                 ...
                 text_area_insert(f"?¥Êí≠?≠Êîæ??FFplay)?üÂ? (PID={proc.pid})", "Live")
             ...
             msg = "?™Ê™¢Ê∏¨Âà∞ mpv/ffmpeg/ffplayÔºåÁÑ°Ê≥ïÊí≠?æÁõ¥?≠Èü≥Ë®?

             print(f"[WS] {msg}")

             text_area_insert(f"?†Ô? {msg}", "Live")
        ...
        try:
                    ...
                    if proc and proc.stdin:
                        ...
                        try:
                            ...
                            if proc.poll() is not None:
                                ...
                                msg = f"?¥Êí≠?≠Êîæ?®‰∏≠??(Code: {proc.returncode})"
                                ...
                                if stderr_out:
                                    ...
                                    text_area_insert(f"?ØË™§Ë®äÊÅØ: {err_str[:200]}", "Live")
            ...
            if RELAY_BROADCASTER_WS == ws:
                ...
                text_area_insert(f"?πÔ? ?¥Êí≠‰æÜÊ??∑Á?", "Live")
...
def api_speak_audio_blob():
    ...
    try:
        ...
        print(f"[Debug] ?∂Âà∞?ÑÈü≥‰∏äÂÇ≥: {save_path} (Size: {os.path.getsize(save_path)} bytes)")
        ...
        if file_size < 1024:
            ...
            msg = f"?ÑÈü≥Ê™îÊ??éÂ? ({file_size} bytes)ÔºåÂèØ?ΩÈ??≥Â§±?óÊ??ÇÈ?Â§™Áü≠"
        ...
        if ext.lower() == ".webm":
            ...
            if not _FFMPEG:
                ...
                err = "‰º∫Ê??®ÁÑ° ffmpegÔºåÁÑ°Ê≥ïË?Ê™î„ÄÇË?ÂÆâË? ffmpeg.exe??
            ...
            text_area_insert(f"?? Ê≠?ú®Â∞?WebM ËΩâÊ???MP3...", "Rec")
            ...
            try:
                ...
                print(f"[Debug] ?ãÂ?ËΩâÊ? WebM -> MP3: {mp3_path}")
                ...
                if os.path.exists(mp3_path) and os.path.getsize(mp3_path) > 0:
                    ...
                    print(f"[Debug] ËΩâÊ??êÂ?, MP3 Size: {sz} bytes")

                    text_area_insert(f"??ËΩâÊ??êÂ? ({sz} bytes)ÔºåÊ??ôÊí≠??, "Rec")

                else:

                    msg = "ËΩâÊ?ÂæåÊ?Ê°àÊ?Â§±Ê?Â§ßÂ???0"
                ...
                text_area_insert(f"??ffmpeg ËΩâÊ?Â§±Ê?Ôºö{err_msg}", "Rec")

                raise RuntimeError(f"ËΩâÊ? MP3 Â§±Ê?: {err_msg}")

            except Exception as cvt_err:

                text_area_insert(f"??ËΩâÊ??ºÁ?‰æãÂ?Ôºö{cvt_err}", "Rec")

                raise RuntimeError(f"ËΩâÊ? MP3 ?ºÁ?‰æãÂ?: {cvt_err}")



        # Save metadata so it appears in file list

        _write_upload_meta(save_path, "?ÑÈü≥-" + get_now().strftime("%H%M%S"), fname, "audio")



        # Queue for playback

        print(f"[Debug] ?ºÂè´ play_mp3_file({save_path})")
            ...
            try:
                ...
                if len(inputs) > 1:
                    ...
                    if mres.returncode == 0 and os.path.exists(merged_mp3) and os.path.getsize(merged_mp3) > 100:
                        ...
                        print(f"[Debug] ?à‰Ωµ?êÂ?: {final_play_path}")

                    else:

                        print(f"[Debug] ?à‰ΩµÂ§±Ê?: {mres.stderr.decode('utf-8', errors='ignore')}")
...
_BASE_FOR_TT = APP_DIR if _writable_dir(APP_DIR) else DATA_DIR  # Ë™≤Ë°®?∫Ê??ÆÈ?

# ?ÅË®±‰∏äÂÇ≥?ÑÂâØÊ™îÂ?ÔºàÂê´?≥Ë?/?ñÁ?/ÂΩ±Á?Ôº?
...
# CWAÔºà‰∏≠Â§ÆÊ∞£Ë±°ÁΩ≤ÔºâÂú∞??API ?ëÈë∞ÔºàÈ?Ë®≠Â??∞Â?ËÆäÊï∏ CWA_API_KEY ËÆÄ?ñÔ??•ÁÇ∫Á©∫Â??Ö‰Ωø??USGSÔº?
...
# == [ANCHOR] Â≠∏Á?Á´ØÊéß?∂È?ÁΩ?==

# ===============================

STUDENT_UDP_LISTEN_PORT = 8080        # ?•Êî∂Â≠∏Á?Á´?HELLO (?πÁÇ∫ 8080 Alternative HTTP)

STUDENT_UDP_CMD_PORT = 8081           # Â≠∏Á?Á´ØÈ?Ë®≠Êé•?∂ÂëΩ‰ª§Á? UDP ??(?πÁÇ∫ 8081 Alternative HTTP)

STUDENT_HELLO_TIMEOUT = 60            # Ë∂ÖÈ?Ê≠§Á??∏Êú™?∂Âà∞ HELLO Ë¶ñÁÇ∫?¢Á?

STUDENT_DISCOVER_INTERVAL = 15        # Âª?í≠ DISCOVER ?ÑÈ??îÔ?ÁßíÔ?

STUDENT_DISCOVER_IP = "255.255.255.255"



# Â≠∏Á?Á´ØÊ??ÆÔ?client_id -> {ip, port, hostname, group, mac, last_seen}
students_clients = {}
students_lock = threading.Lock()
students_stop_event = threading.Event()

# WebSocket Â≠∏Á?Á´ØÔ?client_id -> WebSocket Object
AGENT_WS_CLIENTS = {}
AGENT_WS_LOCK = threading.Lock()

# WebSocket ?èË¶Ω?®Á´ØÔºöList of WebSocket Objects
WEB_WS_CLIENTS = []
...
def get_ports_snapshot_text():
    ...
    if not items: return "(?°ÂèØË¶ãÁ? COM Ë£ùÁΩÆ)"
...
def auto_detect_lcus_port(max_wait_s: float = 8.0):
    ...
    # ?? ÂæπÂ??≤Ê≠¢?åÊôØ?¢Ê∏¨Ë©¶Â??ãÂ? Serial Port
...
def control_usb_relay(status, retries=5):
    ...
    if not RELAY_PORT:
        ...
        ui_safe(lambda: status_label.config(text=f" ???°Ê??µÊ∏¨?∞Áπº?ªÂô® (COM)"))
    ...
    try:
        ...
        with RELAY_LOCK:
            with serial.Serial(RELAY_PORT, 9600, timeout=1.0) as ser:
                ...
                ui_safe(lambda: status_label.config(text=f" Ë™ûÈü≥?üÁî®‰∏≠Ô?ÁπºÈõª?®Ô?{status}"))
        ...
        ui_safe(lambda: status_label.config(text="??ÁπºÈõª?®Êéß?∂Â§±??))
...
def _relay4_set(key, val):
    ...
    # ???åÊ≠•?¥Êñ∞?∞ÂÖ®??STATE ‰ª•‰æø /state API ËøîÂ?
    ...
    # ??Âª?í≠?åÊ≠•ÔºöÁ¢∫‰øùÊ??âÈ???relay4.html ?ÑÈ??¢Âç≥?ÇÊõ¥??
    try:
        import json as _j
        _msg = _j.dumps({
            "type": "relay4_state",
...
def get_manual_relay4_port():
    """Âæ?~/.udp_receiver/relay4_port.txt ?ñÁí∞Â¢ÉË???RELAY4_PORT ËÆÄ??4-Relay COM??""
...
def auto_detect_4relay_port(max_wait_s: float = 8.0):
    ...
    # ?? ÂæπÂ??≤Ê≠¢ 4-Relay ?åÊôØ?¢Ê∏¨
    if manual in ("DISABLED", "NONE"):
        print(">>> [4R] 4-Relay Backend Serial Access is CRITICALLY DISABLED")
        return "DISABLED"

    """4-Relay ?™Â??ÉÊ?ÔºöÁõ°?èÊâæ?å‰??ØÂñÆ Relay?çÁ? CH34x COM??""
    if manual:
...
def list_4relay_candidate_ports():
    ...
    ?óÂá∫?ÆÂ??ØÁî®??4-Relay COM Ê∏ÖÂñÆÔº?

    - ?™Ê???CH34x / USB-SERIAL ?∏È??ÑÂ?

    - ?òÈ??øÈ??ÆÂ?Ê≠?î±??Relay ‰ΩøÁî®??COMÔºàRELAY_PORT ??RELAY_INFO["port"]Ôº?
...
def control_usb_relay4(ch: int, on: bool, retries: int = 5, port_override: str | None = None):
    global RELAY4_PORT
    if ch not in (1, 2, 3, 4): raise ValueError("ch ÂøÖÈ???1~4")

    # [Sync] ?™Ë??á‰ª§?ÅÈ?ÔºåÁ¨¨‰∏Ä?ÇÈ??¥Êñ∞?Ä?ãË?Âª?í≠ (Virtual Sync)
    # ?ôÊ®£?≥‰æø‰º∫Ê??®Ê??•ÂØ¶È´îÔ?Web Serial ?áÊ?ÂæåÔ??∂‰??ªËÖ¶‰πüËÉΩ?≥Ê??ãÂà∞ËÆäËâ≤
    with RELAY4_LOCK:
    ...
    # ?•Ë???DISABLED Ê®°Â?ÔºåÁõ¥?•Ë???(Web Direct Mode)
...
RELAY_AUTO_ON = False
RELAY_OFF_DELAY = 0.5  # ?ÄÁµÇ‰??™Âª∂??(play_sound ?ßÈÉ®Â∑≤Ê? 1.5 Áß?browser Á∑©Ë?)
...
def play_fx(filename, ignore_interrupt=True, wait=True):
    # ?çÂ??ëÂà∞ play_sound ‰ª•‰æøÂª?í≠?∞Â?Á´?
    ...
    def _apply():
        ...
        try:
            ...
            if 'volume_label' in globals():
                ...
                volume_label.config(text=f"?≥È?Ôºö{VOLUME_LEVEL}%")

            status_label.config(text=f" Ë™ûÈü≥?üÁî®‰∏≠Ô??≥È? {VOLUME_LEVEL}%Ôº?)
...
def broadcast_web_audio(filename, duration=0):
    """
    Âª?í≠?≥Ë??≠ÊîæÁµ¶Ê???Web ?®Êà∂ (Â∑≤ÂÑ™?ñÔ??ØÊè¥?çË??éÊøæ)
    """
    basename = os.path.basename(filename)
    
    # Ê±∫Â?Á∂≤È??ØÂ??ñÁ??∏Â?Ë∑ØÂ?
...
def play_sound(filename, duration_estimate=None, ignore_interrupt=False, wait=True):
    print(f"[Speaker] ?≠Êîæ?≥Ë?: {filename} (wait={wait})")
    try:
        # Resolve real path
        real_path = filename
            ...
            try:
                if real_path.lower().endswith(".mp3"):
                    ...
                    try:
                        ...
                        # ?πÊ?‰∫åÔ?‰æùÊ?Ê°àÂ§ßÂ∞è‰º∞ÁÆ?(128kbps ??EdgeTTS Ê®ôÊ?‰ΩçÂ???
                        print(f"[Speaker] mutagen Â§±Ê?: {_mu_err}  ???πÁî®Â§ßÂ?‰º∞Á?")
                        try:
                            ...
                            duration_estimate = 8.0  # ÂÆâÂÖ®?êË®≠??
        ...
        # [KEY FIX] Á≠âÂ??èË¶Ω?®Á´Ø?üÊ≠£?≠Â? (Á∂≤Ë∑ØÂª∂ÈÅ≤ + Á∑©Ë??üÂ? + ?≠ÊîæÊÆòÁ?)
        # ‰∏çÁÆ°?≥Ë??∑Áü≠ÔºåÁÄèË¶Ω?®Â??∂Âà∞?á‰ª§?∞Êí≠ÂÆåÈÉΩ?ÄË¶ÅÈ?Â§ñÊ???
        if not interrupted:
            BROWSER_AUDIO_TAIL = 2.5  # ÁßíÔ?Áµ¶ÁÄèË¶Ω?®Á?È°çÂ?Á∑©Ë??ÇÈ? (Âæ?1.5 Â¢ûÁÇ∫ 2.5)
            tail_t = 0
...
def _interrupt_current_playback():
    ...
    stop_web_audio()
    # Áµ¶‰?‰∏ÄÈªûÊ??ìË? Worker ?ãÂà∞ EventÔºåÁÑ∂ÂæåÂ?Ê∏ÖÈô§ÔºåÈÅø?çË™§ÊÆ∫Á??•ËÄå‰??ÑÊñ∞‰ªªÂ?
    time.sleep(0.1)
...
def _unique_stem_in_uploads(stem: str) -> str:
    ...
    """Á¢∫‰? uploads ?ß‰??çÂ?Ôºà‰ª• .mp3 Ê™¢Êü•ÔºâÔ?ÂøÖË??ÇÂ? (1)??2)??""
...
def download_youtube_audio_to_uploads(url: str) -> tuple[str, float, str]:
    ...
    ‰∏ãË? YouTube ?≥Ë? ??uploads Ë≥áÊ?Â§æÔ?Ê™îÂ?=ÂΩ±Á?Ê®ôÈ?.mp3ÔºåÂ???(mp3_path, duration_sec, title)

    """

    if not _FFMPEG:

        raise RuntimeError("Á≥ªÁµ±?™Êâæ??ffmpegÔºåÁÑ°Ê≥ïË???mp3?ÇË?ÂÆâË? ffmpeg ?ñÊ? ffmpeg.exe ?æÂú®Á®ãÂ??åÁõÆ?Ñ„Ä?)
    ...
    if not os.path.exists(final_mp3):
        ...
        # Â∞ëÊï∏?ÖÊ?ÔºöÂ??ïÁ??ØÊ??ç‰???mp3ÔºàÁ?Ë´ñ‰?‰∏çÊ?ÔºâÔ??öÊ?ÂæåÊ??¥Ê?Â∞?
        ...
        if cand:
            ...
            raise FileNotFoundError("‰∏ãË?ÂÆåÊ?‰ΩÜÊú™?æÂà∞Ëº∏Âá∫Ê™î„Ä?)
...
def play_youtube_audio_with_relay(url):
    ...
    try:
        ...
        set_playing_status(" Ê≠?ú®‰∏ãË? YouTube ?≥Ë?‰∏¶‰?Â≠òËá≥ uploads ...")

        mp3_path, duration, title = download_youtube_audio_to_uploads(url)

        if stop_playback_event.is_set() or voice_muted:

            set_playing_status("?πÔ? ?≠ÊîæÂ∑≤‰∏≠??); ui_safe(_set_progress, 0); STATE["progress"]=0; return

        set_playing_status(f" ‰∏ãË?ÂÆåÊ?Ôºö{os.path.basename(mp3_path)}ÔºåÈ?ÂßãÊí≠?æ‚Ä?)

        text_area_insert(f" YouTube ??uploadsÔºö{os.path.basename(mp3_path)}Ôºà{title}Ôº?)

        save_to_csv(f"PlayYouTubeSaved:{os.path.basename(mp3_path)}", "System")

        _really_play_mp3_file(mp3_path)  # ‰∏çÂà™?§Ô?‰øùÂ???uploads

        if not stop_playback_event.is_set() and not voice_muted:

            set_playing_status("??YouTube?≥Ë??≠ÊîæÂÆåÁï¢ÔºàÊ?Ê°àÂ∑≤‰øùÁ???uploadsÔº?)

            text_area_insert("??YouTube ?≥Ë??≠ÊîæÂÆåÊ?ÔºàÂ∑≤‰øùÁ?Ê™îÊ?Ôº?)

            save_to_csv(f"PlayYouTubeAudioDone:{os.path.basename(mp3_path)}", "System")

        else:

            set_playing_status("?πÔ? ?≠ÊîæÂ∑≤‰∏≠??); ui_safe(_set_progress, 0); STATE["progress"]=0

    except Exception as e:

        print("YouTube ?≠ÊîæÂ§±Ê?Ôº?, e)

        text_area_insert(f"??YouTube ?≠Êîæ/‰∏ãË?Â§±Ê?Ôºö{e}")

        save_to_csv(f"PlayYouTubeAudioFail:{url}", "System")

        set_playing_status("??YouTube?≥Ë??ïÁ?Â§±Ê?"); ui_safe(_set_progress, 0); STATE["progress"]=0
...
def mp3_worker():
    ...
    while True:
        ...
        print(f"[Debug] MP3 Worker ?ñÂá∫: {item}")

        try:

            if stop_playback_event.is_set() or voice_muted:

                print(f"[Debug] ?•È??≠Êîæ (Stop={stop_playback_event.is_set()}, Mute={voice_muted})")
            ...
            if isinstance(item, tuple) and item and item[0] == "URL":
                ...
                set_playing_status(" Ê≠?ú®‰∏ãË? MP3 ...")

                path = _download_mp3_to_temp(url)

                if not path:

                    set_playing_status("??‰∏ãË?/?≠ÊîæMP3Â§±Ê?")
...
def _really_play_mp3_file(path):
    ...
    print(f"[Debug] Ê∫ñÂ??≠Êîæ MP3: {path}")
    ...
    if not os.path.exists(mp3_path):
        print(f"[Debug] Ê™îÊ?‰∏çÂ??? {mp3_path} (Original: {path})")
        return

    set_playing_status(f" ?≠ÊîæMP3‰∏?..({os.path.basename(path)})")


    text_area_insert(f" ?¨Ê? MP3Ôºö{os.path.basename(path)}")
    ...
    try:
        ...
        text_area_insert(f"???≠ÊîæÂ§±Ê?Ôºö{e}", "System")

        

    if not stop_playback_event.is_set() and not voice_muted:

        set_playing_status("???≠ÊîæMP3ÂÆåÊ?")

        text_area_insert("???¨Ê? MP3 ?≠ÊîæÂÆåÊ?")
...
def play_mp3_file(path):
    ...
    print(f"[Debug] ?†ÂÖ• MP3 Queue: {path}")
...
def _download_mp3_to_temp(url: str) -> str | None:
    ...
    text_area_insert(f"‚¨áÔ? ‰∏ãË? MP3Ôºö{url}")
    ...
    try:
        ...
        if not (ct.startswith('audio/') or url.lower().endswith('.mp3')):
            ...
            if not (ct2.startswith('audio/') or url.lower().endswith('.mp3')):
                ...
                raise ValueError("URL ‰∏çÊòØ?ØËæ®Ë≠òÁ??≥Ë?")
        ...
        text_area_insert(f"??‰∏ãË? MP3 Â§±Ê?Ôºö{e}")
...
# == [ANCHOR] Edge/Piper/gTTS/SAPI5 Ë™ûÈü≥ ==
...
def detect_language(text):
    ...
    # ?àÁ?‰ΩøÁî®??UI Âº∑Âà∂?∏Ê?ÔºàÈÅø?çË¢´Â≠óÂ??µÊ∏¨Ë¶ÜË?Ôº?

    # Â¶ÇÊ?‰ΩøÁî®?ÖÈÅ∏‰∫Ü„ÄåËá™?ïÂÅµÊ∏¨„ÄçÔ??çË?‰∏ãÊñπ?ÑÂ??ÉÂÅµÊ∏?

    # ?¶Â??¥Êé•?ûÂÇ≥‰ΩøÁî®?ÖÈÅ∏?ÑË?Ë®Ä‰ª?¢º
    ...
    # Á∞°ÂñÆ?§Â?ÔºöËã•?±Ê?Â≠óÊ?Ë∂ÖÈ?‰∏Ä?äÂ?Ë¶ñÁÇ∫?±Ê?
...
LANG_OPTIONS=[("?™Â??µÊ∏¨ (Auto)","auto"), ("‰∏≠Ê?ÔºàÂè∞???","zh-TW"),("?±Ê?ÔºàÁ??ãÔ?","en-US"),("?•Ê?ÔºàÊó•?¨Ô?","ja-JP"),("?ìÊ?ÔºàÈ??ãÔ?","ko-KR"),("Ë∂äÂ?Ë™?,"vi-VN"), ("?¨Â?ÂØ?È´òÊ?","km-KH"), ("ÂØÆÂ?","lo-LA"), ("Ê≥∞Â?","th-TH"), ("Á∑¨Áî∏","my-MM"), ("È¶¨‰?Ë•ø‰?","ms-MY"), ("?∞Â∞ºË™?,"id-ID"), ("?≤Â?Ë≥?,"tl-PH"), ("?∞Ë?","nan-TW")]
...
def auto_unmute_if_needed():
    ...
    if STATE.get("auto_unmute_on_play", True) and voice_muted:
        ...
        ui_safe(status_label.config, text=" Ë™ûÈü≥?üÁî®‰∏≠Ô??™Â?Ëß?ô§?úÈü≥Ôº?, fg="#188a3a")

        set_playing_status(" Ë™ûÈü≥?üÁî®‰∏?)



@with_relay_playback
def taigi_play_wav_with_fx(path):
    """?∞Ë??àÊ?ÂÆåÊ?ÂæåÔ?Â•óÁî®?éÁõ¥?•Êí≠?æ„ÄèË??ºÔ?Relay ?ã‚??çÂ??íÂÖßÂÆπ‚?ÁµêÊ??íRelay ?ú„Ä?""
    ...
    try:
        ...
        auto_unmute_if_needed()
        ui_safe(set_playing_status, f" ?∞Ë??≠ÊîæÔºö{os.path.basename(path)}")
        # ?çÂ???
        try:
        ...
        play_sound(path) # ‰ΩøÁî®‰∏ªÊí≠?æÂô®ÔºåÊîØ??Web Âª?í≠
        ...
        ui_safe(set_playing_status, "???∞Ë??≠ÊîæÂÆåÊ?")
    finally:
        if relay_acquired:
            relay_release("taigi_play_wav_with_fx")
        text_area_insert(f"???∞Ë??≠Êîæ‰æãÂ?Ôºö{e}")
...
PIPER_CFG = {} # ?®Â?Ë®≠Â?Âø´Â?
PIPER_FORCE = False  # ?èÈ??á‰ª§?®Ê?Âº∑Âà∂ Piper ?™Â?ÔºàË? handle_msgÔº?
...
def _piper_load_cfg():
    ...
    try:
        ...
        # È©óË??¢Ê?Ë∑ØÂ??ØÂê¶?âÊ?ÔºåÁÑ°?àÂ??çÊ?
...
def _piper_run_to_wav(text: str, out_wav: str, lang_code: str = None) -> tuple[bool, str]:
    ...
    # ?ôË£°?ÑÂèØ?®ÊÄßÊ™¢?•Á?ÂæÆÊîæÂØ¨Ô??™Ë???exe Â∞±ÈÄô‰?Ê≠•Â?ÁÆóÈÄöÈ?ÔºåÊ®°?ãÂ??¢Â?‰æÜÊâæ

    exe = PIPER_CFG.get("piper_exe") or ""

    if not (exe and os.path.isfile(exe)):

        return (False, "piper.exe ?™Â∞±Á∑?)
    ...
    if lang_code:
        ...
        if found:
            ...
            # ?•Êâæ?∞Á?Ê®°Â?Ë∑üÈ?Ë®≠‰??åÔ??ØÂú® log ?êÁ§∫ÔºàÈÅ∏?®Ô?

    

    if not (use_model and os.path.isfile(use_model)):

        return (False, f"?™Êâæ?∞ÂèØ?®Ê®°??(lang={lang_code}, default={default_model})")
    ...
    if not os.path.isfile(use_model + ".json"):
        ...
        return (False, f"Ê®°Â?Ë®≠Â?Ê™îÈÅ∫Â§? {use_model}.json")
    ...
    if spk:
        ...
        # Ê≥®Ê?ÔºöËã•?áÊ?Ê®°Â?ÔºåÂ? speaker ID ?ØËÉΩ‰∏çÈÅ©?®Ô??ôË£°?´Ê?‰øùÁ?
    ...
    try:
        ...
        if should_chime:
            if not (START_SOUND and os.path.exists(START_SOUND)):
                text_area_insert(f"?†Ô? ?çÂ??≥Ê?Ê°à‰?Â≠òÂú®: {START_SOUND}", "TTS")
            if not (END_SOUND and os.path.exists(END_SOUND)):
                text_area_insert(f"?†Ô? ÁµêÊ??≥Ê?Ê°à‰?Â≠òÂú®: {END_SOUND}", "TTS")
        # Parse Per-Message Metadata: {{L=xx|G=xx}}text
        local_lang = None
        local_gender = None
        
        # [Safety] Strip sender tag if present (e.g. "@API_V2: ...")
        # Matches "@Tag: " or "@TagÔº?" at start
        if text.startswith("@"):
            import re
            text = re.sub(r"^@[\w_]+[:Ôºö]\s*", "", text)
        ...
        stop_playback_event.clear()  # ?êÈÅø?çÂ?Â∞éÈü≥Ë¢´Ê??ôÁ??úÊ≠¢?óÊ??ìÊñ∑
        if stop_playback_event.is_set() or voice_muted:
            return
        ui_safe(set_playing_status, " ?óË?‰∏≠Ô?")
        ui_safe(_set_progress, 0); STATE["progress"] = 0
        ...
        # ?∞Ë?Ë∑ØÂ?Ôºö‰∫§Áµ¶Â??®ÂáΩÂºèÊí≠?æÔ??øÂ??çË??çÂ?/ÁµêÊ???
        ...
        if PIPER_FORCE and _piper_available():
            try:
                ui_safe(set_playing_status, "?? ?óË?‰∏?(Piper Force)...")
                ...
                if not ok:
                    text_area_insert(f"??Piper ?àÊ?Â§±Ê?ÔºàforceÔºâÔ?{log}", "TTS")
                else:
                    # [Chime] Piper Force
                    ...
                    if not (stop_playback_event.is_set() or voice_muted):
                        try:
                            ...
                            pass
                        ui_safe(set_playing_status, "???óË?ÂÆåÊ?ÔºàPiper ?¢Á?ÔΩúforceÔº?)
                    return

            except Exception as e:
                text_area_insert(f"??Piper force ‰æãÂ?Ôºö{e}", "TTS")
        ...
        if TRIAL_EXPIRED:
            text_area_insert("?†Ô? Ë©¶Áî®?üÂ∑≤?éÔ??ÖËÉΩ‰ΩøÁî® gTTS ?ôÊè¥", "TTS")
        elif azure_key and azure_region:
            ui_safe(set_playing_status, "?? ?óË?‰∏?(Azure TTS)...")
            try:
                ...
                if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
                    ...
                    if not (stop_playback_event.is_set() or voice_muted):
                        ...
                        ui_safe(set_playing_status, f"???óË?ÂÆåÊ?ÔºàAzure Official: {v_azure}Ôº?)
                    return
                else:
                    cancellation_details = result.cancellation_details
                    text_area_insert(f"?†Ô? Azure Speech Â§±Ê?Ôºö{cancellation_details.reason} - {cancellation_details.error_details}", "TTS")

            except ImportError:
                 text_area_insert("?†Ô? ?™Â?Ë£?azure-cognitiveservices-speechÔºåË∑≥??Azure ÂÆòÊñπË∑ØÂ?", "TTS")
            except Exception as e:
                 text_area_insert(f"?†Ô? Azure Speech ‰æãÂ?Ôºö{e}", "TTS")
        ...
        if not TRIAL_EXPIRED and should_use_edge:
             for v, r in trials:
                try:
                    ...
                    ui_safe(set_playing_status, f"?? ?óË?‰∏?(EdgeTTS: {v})...")
                    ...
                    play_sound(mp3_path)
                    # [Safety] Âª∂ÈÅ≤?™Èô§ÔºåË? Web client ?âÊ??ì‰?Ëº?
                    def delayed_cleanup(p):
                        try:
                            time.sleep(10)
                            if os.path.exists(p): os.remove(p)
                    ...
                    if not (stop_playback_event.is_set() or voice_muted):
                        try:
                            ...
                            pass
                        ui_safe(set_playing_status, f"???óË?ÂÆåÊ?Ôºà{v}{'' if r is None else f', {r}'}Ôº?)
                    ...
                    if "SSL" in err_str or "CERTIFICATE" in err_str or "ClientConnectorError" in err_str:
                         ...
                         try:
                             ...
                             try:
                                 ...
                                 ui_safe(set_playing_status, f"?? ?óË?‰∏?(EdgeTTS Unverified: {v})...")
                                 if r is not None:
                                     tts = edge_tts.Communicate(text=text, voice=v, rate=r, volume='+50%')
                                 else:
                                     tts = edge_tts.Communicate(text=text, voice=v, volume='+50%')
                                 ...
                                 if not (stop_playback_event.is_set() or voice_muted):
                                     ui_safe(set_playing_status, f"???óË?ÂÆåÊ?ÔºàUnverified SSL: {v}Ôº?)
                                     STATE["edge_tts_fails"] = 0
                                 return # Success
                             finally:
                    ...
                    reason = str(e)
                    text_area_insert(f"?†Ô? Edge TTS Â§±Ê?Ôºàvoice={v}, rate={r}ÔºâÔ?{reason}", "TTS")
                    continue
        ...
        text_area_insert("?πÔ? Edge TTS ‰∏çÂèØ?®Ô?ËΩâÁî® gTTS ?ôÊè¥??, "TTS")
        ...
        try:
            ...
            if not (stop_playback_event.is_set() or voice_muted):
                ...
                ui_safe(set_playing_status, "???óË?ÂÆåÊ?ÔºàgTTS ?ôÊè¥Ôº?)

            return

        except Exception as e:

            text_area_insert(f"??gTTS ?ôÊè¥Â§±Ê?Ôºö{e}ÔºåÂ?Ë©?Piper...", "TTS")
        ...
        if not TRIAL_EXPIRED and _piper_available():
            text_area_insert(f"?πÔ? gTTS ‰∏çÂèØ?®Ô?ËΩâÁî® Piper ?ôÊè¥??, "TTS")
            try:
                with tempfile.NamedTemporaryFile(delete=False, suffix=".wav") as fp:
                    wav_path = fp.name
                ok, log = _piper_run_to_wav(text, wav_path, lang_code=lang)
                if ok:
                    ...
                    if not (stop_playback_event.is_set() or voice_muted):
                        try:
                            ...
                            pass
                        ui_safe(set_playing_status, "???óË?ÂÆåÊ?ÔºàPiper ?¢Á?Ôº?)
                    return
                else:
                    text_area_insert(f"??Piper ?àÊ?Â§±Ê?Ôºö{log}", "TTS")
            except Exception as e:
                text_area_insert(f"??Piper ‰æãÂ?Ôºö{e}", "TTS")
        else:
             # Debug info for user
             pe = PIPER_CFG.get("piper_exe") or "?™Ë®≠ÂÆ?
             pm = PIPER_CFG.get("model") or "?™Ë®≠ÂÆ?
             text_area_insert(f"?†Ô? Piper Â∞öÊú™Â∞±Á?ÔºàExe: {pe}, Model: {pm}Ôº?, "TTS")
        ...
        try:
            ...
            if not (stop_playback_event.is_set() or voice_muted):
                ...
                ui_safe(set_playing_status, "???óË?ÂÆåÊ?ÔºàSAPI5 ?¢Á?Ôº?)
        ...
        ui_safe(set_playing_status, "???óË?Â§±Ê?")
...
def show_qr_popup(title_text: str, url_text: str):
    ...
    if not _HAS_QR:
        ...
        try:
            ...
            messagebox.showinfo(title_text, f"URLÔºö{url_text}\n\nÔºàÊú™ÂÆâË? qrcode/PillowÔºåÁÑ°Ê≥ïÈ°ØÁ§?QRÔº?)
    ...
    try:
        ...
        tk.Label(win, text=f"QR ?üÊ?Â§±Ê?Ôºö{e}", bg=THEME["bg_card"], fg="#EF4444").pack(padx=16, pady=12)
    ...
    def _copy():
        ...
        try:
            ...
            messagebox.showinfo("Â∑≤Ë?Ë£?, "URL Â∑≤Ë?Ë£ΩÂà∞?™Ë≤ºÁ∞?)
    ...
    tk.Button(btns, text="Ë§áË£Ω URL", command=_copy, bg="#F1F5F9", relief="flat", font=THEME["font_s"]).pack(side="left", padx=6)

    tk.Button(btns, text="?®ÁÄèË¶Ω?®È???, command=_open, bg="#D1FAE5", relief="flat", font=THEME["font_s"]).pack(side="left", padx=6)

    tk.Button(btns, text="?úÈ?", command=win.destroy, bg="#FEE2E2", relief="flat", font=THEME["font_s"]).pack(side="left", padx=6)
...
def open_frontend_and_qr_popup():
    ...
    if DISABLE_WEB:
        ...
        try:
            ...
            messagebox.showwarning("?çÁ´Ø?™Â???, "?ÆÂ?Ë®≠Â???DISABLE_WEB=TrueÔºåÊú™?üÂ? Flask ‰º∫Ê??®„Ä?)
    ...
    try:
        ...
        ui_safe(ngrok_status_label.config, text=f" ?çÁ´ØÔºö{url}")

    except Exception:

        pass

    show_qr_popup("?çÁ´ØÁ∂≤Â? QR", url)
...
def generate_taigi_tts(text, gender=None, speed_percent=None):
    """
    ?¢Á??∞Ë?Ë™ûÈü≥Ê™?(‰∏çÊí≠???ÇÊ®°‰ª?taigi_edu.html Â∞àÁî®Ê®°Á??πÂ? (model6)??
    ?ûÂÇ≥: Ê™îÊ?ÁµïÂ?Ë∑ØÂ?
    """
    # Ê±∫Â??≤Âà•
    if gender:
    ...
    try:
        # ?ºÂè´È´òÈü≥Ë≥™Â??êÈ?Ëº?(Â∑≤‰∏≤??TaigiTTSClient)
...
def play_taigi_tts(text):
    """Ê≠§Ë?Ê®°‰ªø taigi_edu.html ?Ñ„ÄåÁôº?≤Ê®°ÁµÑ„ÄçÊñπÂºèÔ?È´òÈü≥Ë≥™Â???+ ?¥Êé•Âª?í≠??""
    try:
        # ?™Â??µÊ∏¨?ØÂê¶?ÄË¶ÅÁøªË≠?(?•Ê?Â≠óÁÇ∫?ãË??áÂ?ËΩâÂè∞Ë™ûÔ?Ê®°‰ªø?ôËÇ≤Ê®°Á?ÊµÅÁ?)
        def _is_mostly_mandarin(t):
            # Á∞°ÂñÆ?§Â?ÔºöËã•?°Âè∞Ë™ûÁâπ?âÊº¢Â≠?Á¨¶Ë?‰∏îÊòØ‰∏≠Ê?ÔºåÂ??óË©¶ÁøªË≠Ø
            taigi_markers = ["??,"??,"??,"‰ø?,"‰π?,"??,"??,"??,"??,"??,"??,"??,"Â∫?,"??,"??,"??,"ÊØ?,"Ë¢?,"√™"]
        ...
        if _is_mostly_mandarin(text):
            try:
                # ?ºÂè´ÁøªË≠Ø API (zh2nan)
                # ‰ΩøÁî®Â∑≤Ê??èËºØÔºåÂ?Ë®?API Key Ê≠?¢∫
        ...
        text_area_insert(f"?†Ô? ?∞Ë?Âª?í≠Â§±Ê?Ôºö{e}", "TTS")
...
def show_fullscreen_message(msg):
    ...
    tk.Label(frame, text=" Á∑äÊÄ•Ë??ØÈÄöÁü•", font=("Microsoft JhengHei UI", 44, "bold"), fg="#38BDF8", bg="#0F172A").pack(pady=(90,40))
    ...
    tk.Label(frame, text="ÔºàÈ??ä„ÄÅÊ?‰ªªÊ??µÊ???Esc ?úÈ?Ôº?, font=("Microsoft JhengHei UI", 22), fg="#94A3B8", bg="#0F172A").pack(pady=(40,30))
...
def _load_timetable_from_disk():
    ...
    try:
        ...
        if not loaded and os.path.isfile(path_csv):
            ...
            sample = {
                ...
                "items": [
                    ...
                    {"dow":1, "time":"07:58", "action":"Bell:ClassStart", "label":"‰∏äË™≤??},

                    {"dow":1, "time":"08:00", "action":"ShowMsg:?©Â?ÔºåÂ?Â≠∏ÂÄëÔ?", "label":"Ë®äÊÅØ"},

                    {"dow":1, "time":"10:00", "action":"PlayMP3:Award.mp3", "label":"?íÁ??≥Ê?"},
        ...
        text_area_insert(f"??ËºâÂÖ•Ë¶èÂ?‰ªªÂ?Â§±Ê?Ôºö{e}")
    ...
    if 'timetable_status_var' in globals():
        ...
        ui_safe(timetable_status_var.set, f" Ë¶èÂ?‰ªªÂ?Ôºö{'?üÁî®' if timetable_enabled else '?úÁî®'}ÔºåÂ∑≤ËºâÂÖ• {STATE['timetable']['count']} Á≠?)

    if 'timetable_path_var' in globals():

        ui_safe(lambda: timetable_path_var.set(f"Ë∑ØÂ?Ôºö{STATE['timetable']['path']}"))
...
def update_next_label():
    ...
    try:
        ...
        if not timetable_enabled:
            ...
            ui_safe(timetable_next_var.set, " ‰∏ã‰?Ê¨°Ô??îÔ??™Â??≠Â†±?úÈ?Ôº?); return

        nxt = compute_next_ring()

        if not nxt:

            ui_safe(timetable_next_var.set, " ‰∏ã‰?Ê¨°Ô???)

        else:

            ymd, hhmm, label = nxt

            disp = f" ‰∏ã‰?Ê¨°Ô?{ymd} {hhmm}" + (f"ÔΩú{label}" if label else "")
...
def _trigger_action(action: str, label: str = "", src: str = "Timetable", idx: int | None = None):
    ...
    text_area_insert(f" Ë¶èÂ?‰ªªÂ?Ëß∏ÁôºÔºà{sender}ÔºâÔ?{label or action}")
    ...
    try:
        ...
        text_area_insert(f"??Ë¶èÂ?‰ªªÂ?Ëß∏ÁôºÂ§±Ê?Ôºö{e}")
...
def timetable_scheduler_loop():
    ...
    while True:
        ...
        try:
            ...
            text_area_insert(f"?†Ô? Ë¶èÂ?‰ªªÂ??íÁ??®ÈåØË™§Ô?{e}")
...
def timetable_play_index(idx: int):
    ...
    text_area_insert(f"?†Ô? Ë¶èÂ?‰ªªÂ?Á¥¢Â?Ë∂ÖÂá∫ÁØÑÂ?Ôºö{idx}")



# ===== ?≤Ê≠¢?çÁ´Ø/ÂæåÁ´Ø?åÊ??†Ê?Âæ™Áí∞?ûÈÄÅÔ??ªÊ?/?ªÈ?Ôº?=====
...
def _is_duplicate_message(sender_ip: str | None, text: str) -> bool:
    ...
    # ?®Ëû¢ÂπïË??ØÔ??´Êí≠???úÈü≥Ôºâ‰??öÂéª?çÔ??øÂ?????âÈ?Ë¢´ÂøΩ??
...
def handle_msg(text, addr):
    ...
    # -------------------------------------------------
    # 1Ô∏è‚É£ ?¥Êé•?≠Êîæ?†Â?Ë°®ÂÖß?ÑÊ?‰ª§Ô?‰∏çËµ∞ audio_proxyÔº?
    # -------------------------------------------------
    if text in CMD_SOUND_TABLE:
        ...
        if os.path.isfile(abs_path):
            ...
            print(f"[WARN] ?≥Ê?‰∏çÂ??? {abs_path}")
        return

    # -------------------------------------------------
    # 2Ô∏è‚É£ ?∂È? Bell / PlayMP3 ?á‰ª§‰ªçËµ∞?üÊú¨??broadcast_web_audioÔºà‰??ôË?Ë°åÁÇ∫Ôº?
    # -------------------------------------------------
    ...
    if text.strip() == "WeatherReport":
        ...
        text_area_insert(f"?∂Âà∞Ê∞?±°?≠Â†±Ë´ãÊ? from {sender}")
        ...
        text_area_insert(f"Ê∞?±°?≠Â†±?ßÂÆπÔºö{text}")
    ...
    if text == "ScheduleReload":
        ...
        text_area_insert(" Â∑≤È??∞Ë??•Ë?Âæã‰ªª??)
    ...
    if text == "RelayRescan":
        text_area_insert(f" ?çÊñ∞?ÉÊ? USB ÁπºÈõª?®Ë?ÁΩ?(by {sender})")
    ...
    if text == "ScheduleEnable":
        ...
        if 'timetable_status_var' in globals(): ui_safe(timetable_status_var.set, " Ë¶èÂ?‰ªªÂ?ÔºöÂ???)
        return
    ...
    if text.startswith("SchedulePlay:"):
        ...
        try:
            ...
            text_area_insert("?†Ô? SchedulePlay ?ÉÊï∏?ØË™§")
    ...
    if text.startswith("YTFull:"):
        ...
        text_area_insert(f"ÔºàÊ?Á§∫Ô?Ë´ãÊîπ?®‰?ËºâÈü≥Ë®äÊí≠?æÊ??®Â?Á´ØÊí≠?æÂô®?®Ëû¢ÂπïÔ?{text}")
    ...
    if text.strip() == "YTClose":
        ...
        text_area_insert(" Â∑≤Êé•??YTCloseÔºàÁ∞°?ñÁ??™Á∂≠?ÅÂ??®Êí≠?æÂô®?Ä?ãÔ?")

        set_playing_status("?πÔ? Â∑≤È??âÂÖ®?¢Â? YouTubeÔºàÊ?Á§∫Ô?")
    ...
    if text.startswith("SetVolume:"):
        ...
        try:
            ...
            text_area_insert(f" ?≥È?Ë®≠ÁÇ∫ {STATE['volume']}%Ôºà‰???{sender}Ôº?)
    ...
    if text.strip() == "VolUp":
        ...
        set_volume(STATE["volume"] + 5); auto_unmute_if_needed(); save_to_csv("VolUp", sender, ip=sender_ip); text_area_insert(f" ?≥È?ÔºãËá≥ {STATE['volume']}%"); return

    if text.strip() == "VolDown":

        set_volume(STATE["volume"] - 5); auto_unmute_if_needed(); save_to_csv("VolDown", sender, ip=sender_ip); text_area_insert(f" ?≥È?ÔºçËá≥ {STATE['volume']}%"); return
    ...
    if text.startswith("PlayTaigi:"):
        ...
        text_area_insert(f"Ô∏??∞Ë??≠Â†±Ôºö{taigi_content}")
    ...
    if text.startswith("lang:tw|") or text.startswith("lang:nan|"):
        ...
        text_area_insert(f"Ô∏??∂Âà∞?∞Ë?Âª?í≠Ôºö{taigi_content}")
    ...
    if text.startswith("ShowMsg:"):
        ...
        text_area_insert(f"?êÂÖ®?¢Â??âËÅ≤?ë‰???{sender}Ôºö{msg_content}")
    ...
    if text.startswith("SilentMsg:"):
        ...
        text_area_insert(f"?êÂÖ®?¢Â??°ËÅ≤?ë‰???{sender}Ôºö{msg_content}")
    ...
    if text == "RelayOn":
        ...
        text_area_insert(f" ?∂Âà∞ RelayOn ?á‰ª§Ôºà‰???{sender}Ôº?)
    ...
    if text == "RelayOff":
        ...
        text_area_insert(f" ?∂Âà∞ RelayOff ?á‰ª§Ôºà‰???{sender}Ôº?)
    ...
    if text in ("CancelALL", "CancelAll", "ForceClear", "StopAll"):
        ...
        set_playing_status("?πÔ? Â∑≤Âº∑?∂Â?Ê∂??úÊ≠¢")

        ui_safe(_set_progress, 0); STATE["progress"] = 0

        ui_safe(status_label.config, text="?πÔ? Â∑≤Ë¢´?†Á´ØÂº∑Âà∂?úÊ≠¢", fg="#b31c2a")

        text_area_insert(f"?πÔ? {sender} ?ºÈÄÅÂº∑?∂Â?Ê∂àÂëΩ‰ª§Ô?Ôºà‰??óÂ∑≤Ê∏ÖÁ©∫Ôº?)
    ...
    if text.strip() == "Bell:ClassStart":
        ...
        auto_unmute_if_needed(); play_mp3_file("ClassStart.mp3"); text_area_insert(" ‰∏äË™≤?¥Êí≠??ClassStart.mp3"); save_to_csv("Bell:ClassStart", sender, ip=sender_ip); return

    if text.strip() == "Bell:ClassEnd":
        print(f"[DEBUG] Quick Command: ClassEnd")
        auto_unmute_if_needed(); play_mp3_file("ClassEnd.mp3"); text_area_insert(" ‰∏ãË™≤?¥Êí≠??ClassEnd.mp3"); save_to_csv("Bell:ClassEnd", sender, ip=sender_ip); return

    if text.strip() == "Bell:EarthquakeAlarm":
        print(f"[DEBUG] Quick Command: EarthquakeAlarm")
        auto_unmute_if_needed(); play_mp3_file("justearthquakeAlarm.mp3"); text_area_insert(" ?∞È?Ë≠¶Â†±?≠Êîæ justearthquakeAlarm.mp3"); save_to_csv("Bell:EarthquakeAlarm", sender, ip=sender_ip); return



    if text.strip() == "MP3Pause":
        pause_web_audio()
        set_playing_status("?∏Ô? MP3 ?´Â? (Web Redirect)")
        return
    ...
    if text.strip() == "MP3Resume":
        resume_web_audio()
        set_playing_status("?∂Ô? MP3 ÁπºÁ? (Web Redirect)")
        return
    ...
    if text.strip() == "MP3Stop":
        stop_web_audio()
        ui_safe(_set_progress, 0); STATE["progress"] = 0
        set_playing_status("?πÔ? MP3 ?úÊ≠¢ (Web Redirect)")
        return
    ...
    if text.strip() == "MP3Seek":
        ...
        text_area_insert("?πÔ? MP3SeekÔºöpygame ‰∏çÊîØ??SeekÔºåÂ∑≤ÂøΩÁï•")
    ...
    if "youtube.com/watch" in text or "youtu.be/" in text or "/shorts/" in text:
        ...
        auto_unmute_if_needed(); text_area_insert(f" ‰∏ãË?‰∏¶Êí≠??YouTube ?≥Ë?Ôºö{text}")
    ...
    if text in ("Boy", "??, "?∑ËÅ≤"):
        def _to_boy():
            if 'gender_label_var' in globals(): gender_label_var.set("?∑ËÅ≤")
            update_voice()
        ui_safe(_to_boy)
        return

    if text in ("Girl", "Â•?, "Â•≥ËÅ≤"):
        def _to_girl():
            if 'gender_label_var' in globals(): gender_label_var.set("Â•≥ËÅ≤")
            update_voice()
        ui_safe(_to_girl)
        return
    ...
    if text in ("Mute", "?úÈü≥"):
        ...
        stop_web_audio()
        ui_safe(status_label.config, text=" Ë™ûÈü≥Â∑≤È??≥Ô??≠Êîæ‰∏≠Êñ∑Ôº?, fg="#888")
        set_playing_status(" Ë™ûÈü≥Â∑≤È???); ui_safe(_set_progress, 0); STATE["progress"] = 0; save_to_csv("Mute", sender, ip=sender_ip); return

    if text in ("Unmute", "Ëß?ô§?úÈü≥", "?ñÊ??úÈü≥"):

        voice_muted = False; STATE["muted"] = False; stop_playback_event.clear()

        ui_safe(status_label.config, text=" Ë™ûÈü≥?üÁî®‰∏?, fg="#188a3a")

        set_playing_status(" Ë™ûÈü≥?üÁî®‰∏?); save_to_csv("Unmute", sender, ip=sender_ip); return
    ...
    if text.startswith("SetRate:"):
        ...
        try:
            ...
            ui_safe(rate_scale.set, v); ui_safe(rate_label.config, text=f"Ë™ûÈÄüÔ?{v}%")
    ...
    if text.startswith("SetMeloSpeaker:"):
        ...
        text_area_insert(f" Melo ?çÈü≥?òËÆæ‰∏?{spk}ÔºàÊù•??{sender}Ôº?)
        return
    ...
    if text.startswith("SetMeloEnabled:"):
        ...
        status_msg = "Â∑≤Â??? if USE_MELO_TTS else "Â∑≤Â???
        text_area_insert(f" Melo AI ËØ≠Á≥ª {status_msg}ÔºàÊù•??{sender}Ôº?)
        return
    ...
    if text.startswith("PlayWithChime:"):
        ...
        text_area_insert(f" ‰æÜËá™ {sender}ÔºöPlayWithChime ??{arg}")
        ...
        if final_path and os.path.exists(final_path) and _FFMPEG:
            ...
            try:
                ...
                if len(inputs) > 1:
                    ...
                    text_area_insert(f"?? ?ïÁ??≥Ê??à‰Ωµ ({len(inputs)} clips)...")
                    ...
                    if res.returncode != 0:
                        ...
                        text_area_insert(f"??FFMPEGÂ§±Ê?: {err}")
                    ...
                    if os.path.exists(merged_mp3) and os.path.getsize(merged_mp3) > 100:
                        ...
                        text_area_insert("???à‰Ωµ?êÂ?ÔºåÈ?ÂßãÊí≠??)

            except Exception as e:

                print(f"[Error] PlayWithChime merge exception: {e}")

                text_area_insert(f"???à‰Ωµ‰æãÂ?: {str(e)}")
    ...
    if text.startswith("PlayMP3:"):
        ...
        text_area_insert(f" ‰æÜËá™ {sender}ÔºöPlayMP3 ??{arg}")
    ...
    if text.startswith("PlayChime:"):
        ...
        if mode == "start":
            ...
            text_area_insert(f" ?≠Êîæ?ãÂ??êÁ§∫??-> Relay ON")
        ...
        if target:
            ...
            try:
                ...
                if os.path.exists(path):
                    ...
                    text_area_insert(f"?†Ô? ?êÁ§∫?≥Ê?‰∏çÂ??®Ô?{target}")

            except Exception as e:

                text_area_insert(f"???êÁ§∫?≥Êí≠?æÂ§±?óÔ?{e}")
        ...
        if mode == "end":
            ...
            text_area_insert(f" ÁµêÊ??≥Êí≠??-> Relay OFF")
    ...
    if text.startswith("PiperForce:"):  # PiperForce:1 / PiperForce:0
        ...
        text_area_insert(f" Piper Âº∑Âà∂?™Â?Ôºö{PIPER_FORCE}")
    ...
    if text.startswith("PiperSet:"):  # ‰æãÔ?PiperSet: length=1.05, noise=0.6, noisew=0.8, speaker=0
        ...
        try:
            ...
            text_area_insert(f" Piper ?ÉÊï∏?¥Êñ∞Ôºö{PIPER_CFG}")

        except Exception as e:

            text_area_insert(f"??PiperSet Ëß??Â§±Ê?Ôºö{e}")
    ...
    text_area_insert(f"‰æÜËá™ {sender}Ôºö{display_text}"); save_to_csv(text, sender, ip=sender_ip)
    ...
    if TRIAL_EXPIRED:
        text_area_insert("?†Ô? Ë©¶Áî®?üÂ∑≤?éÔ??°Ê??∑Ë?Ë™ûÈü≥?àÊ?", "TTS")
        return
...
def student_udp_listener():
    ...
    try:
        ...
        text_area_insert(f"[Student Controller] UDP ??ÅΩ?üÂ???0.0.0.0:{STUDENT_UDP_LISTEN_PORT}", "StudentCtrl")

    except Exception as e:

        text_area_insert(f"[Student Controller] UDP ??ÅΩÂ§±Ê?Ôºö{e}", "StudentCtrl")
...
def student_broadcast_discover():
    ...
    """ÂÆöÊ?Âª?í≠ DISCOVERÔºåÊ??íÂ≠∏?üÁ´Ø?ûÂÇ≥ HELLO"""
    ...
    text_area_insert(f"[Student Controller] DISCOVER Âª?í≠?üÂ?ÔºàÊ? {STUDENT_DISCOVER_INTERVAL} ÁßíÔ?", "StudentCtrl")
...
def send_student_udp_command(ip: str, port: int, cmd: str, args: str = "", client_id: str = None):
    """?ºÈÄÅÊéß?∂Ê?‰ª§Áµ¶Â≠∏Á?Á´?(?™Â?Ëµ?WebSocket)"""
    msg = f"CMD|{cmd}|{args}"
...
app.config['PERMANENT_SESSION_LIFETIME'] = timedelta(hours=2)  # 2Â∞èÊ??éÊ?

app.config['SESSION_COOKIE_HTTPONLY'] = True

app.config['SESSION_COOKIE_SECURE'] = False  # ?¨Âú∞?ãÁôºË®≠ÁÇ∫ FalseÔºåÁ??¢Áí∞Â¢ÉÊîπ??TrueÔºàÈ? HTTPSÔº?
...
def verify_password(password, password_hash):
    ...
    if not _HAS_BCRYPT:
        ...
        # Â¶ÇÊ?Ê≤íÊ? bcryptÔºå‰Ωø?®Á∞°?ÆÊ?Â∞çÔ?‰∏çÂ??®Ô??ÖÁî®?ºÈ??ºÔ?
...
def hash_password(password):
    ...
    if not _HAS_BCRYPT:
        ...
        # Â¶ÇÊ?Ê≤íÊ? bcryptÔºåË??ûÊ??áÔ?‰∏çÂ??®Ô??ÖÁî®?ºÈ??ºÔ?
...
def _is_api_request():
    ...
    """?§Êñ∑?∂Â?Ë´ãÊ??ØÂê¶Â±¨Êñº APIÔºåÈ?Ë¶ÅÂ???JSON"""
    ...
    def decorated_function(*args, **kwargs):
        ...
        if 'user_id' not in session:
            ...
            if _is_api_request():
                ...
                return jsonify(ok=False, error="?™Áôª??, need_login=True), 401
...
def admin_required(f):
    """Ë¶ÅÊ?ÁÆ°Á??°Ê??ôÂ∏´Ê¨äÈ??ÑË?È£æÂô® (Á§∫Á?Ê®°Â?ÔºöÂº∑Ë°åËß£??"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        # Â±ïÁ§∫Ê®°Â?Ôºö‰?Ê™¢Êü• sessionÔºåÁõ¥?•Â?Ë®±Â???
        return f(*args, **kwargs)
    return decorated_function
...
@app.route('/login')
def login_page():
    """?ªÈ??ÅÈù¢ (Á§∫Á?Ê®°Â?ÔºöÁõ¥?•ÈÄ≤ÂÖ•‰∏ªÊéß??"""
    # Â±ïÁ§∫Ê®°Â?‰∏ã‰?È°ØÁ§∫?ªÈ??ÅÈù¢ÔºåÁõ¥?•Ë∑≥ËΩâÂà∞‰∏ªÊéß??
...
@app.post('/auth/login')
def login_api():
    """Â™íÈ?/?üËÉΩ ?ªÂÖ• API (Á§∫Á?Ê®°Â?ÔºöËá™?ïÈÄöÈ?)"""
    session['user_id'] = 'demo_admin'
    session['username'] = 'Â±ïÁ§∫ÁÆ°Á???
    session['role'] = 'admin'
    session.permanent = True
    return jsonify(ok=True, message="Â±ïÁ§∫Ê®°Â??ªÈ??êÂ?", role='admin', username='Â±ïÁ§∫ÁÆ°Á???, user_id='demo_admin')

@app.post('/auth/logout')
def auth_logout():
    """?ªÂá∫ API (Á§∫Á?Ê®°Â?ÔºöÊ?Á©∫‰?È°ØÁ§∫?êÂ?)"""
    session.clear()
    return jsonify(ok=True, message="?ªÂá∫?êÂ?")

@app.get('/auth/status')
def auth_status():
    """Ê™¢Á¥¢?∂Â??®Êà∂?ªÈ??Ä??(Á§∫Á?Ê®°Â?ÔºöÊ∞∏?†Âú®Á∑?"""
    return jsonify(
        ok=True,
        logged_in=True,
        user={
...
def auth_heartbeat():
    ...
    """Session ÂøÉË∑≥Ôºå‰??ÅÁôª?ÑÁ???""

    session.modified = True  # ?¥Êñ∞ session ?ÇÈ?
...
def auth_change_password():
    ...
    try:
        ...
        if not old_password or not new_password:
            ...
            return jsonify(ok=False, error="Ë´ãÊ?‰æõË?ÂØÜÁ¢º?åÊñ∞ÂØÜÁ¢º"), 400

        

        if len(new_password) < 6:

            return jsonify(ok=False, error="?∞Â?Á¢ºËá≥Â∞ëÈ?Ë¶??ãÂ?Á¨?), 400
        ...
        if not user:
            ...
            return jsonify(ok=False, error="?®Êà∂‰∏çÂ???), 404
        ...
        if not verify_password(old_password, user.get('password_hash', '')):
            ...
            return jsonify(ok=False, error="?äÂ?Á¢ºÈåØË™?), 401
        ...
        try:
            ...
            text_area_insert(f"?? ?®Êà∂ {user.get('username')} ‰øÆÊîπ‰∫ÜÂ?Á¢?, "Auth")
        ...
        return jsonify(ok=True, message="ÂØÜÁ¢º‰øÆÊîπ?êÂ?")
        ...
        return jsonify(ok=False, error=f"‰øÆÊîπÂØÜÁ¢ºÂ§±Ê?Ôºö{str(e)}"), 500
...
def auth_admin_create_user():
    ...
    try:
        ...
        if not username or not password:
            ...
            return jsonify(ok=False, error="Ë´ãÊ?‰æõÁî®?∂Â??åÂ?Á¢?), 400
        ...
        # Ê™¢Êü•?®Êà∂?çÊòØ?¶Â∑≤Â≠òÂú®

        if any(u.get('username') == username for u in users):

            return jsonify(ok=False, error="?®Êà∂?çÂ∑≤Â≠òÂú®"), 400
        ...
        try:
            ...
            text_area_insert(f"?ë§ ÁÆ°Á???{session.get('username')} ?µÂª∫‰∫ÜÊñ∞?®Êà∂Ôºö{username} ({role})", "Auth")
        ...
        return jsonify(ok=True, user=return_user, message="?®Êà∂?µÂª∫?êÂ?")
        ...
        return jsonify(ok=False, error=f"?µÂª∫?®Êà∂Â§±Ê?Ôºö{str(e)}"), 500
...
def controller_api_send():
    ...
    if not targets:
        ...
        return jsonify(ok=False, error="Ë´ãËá≥Â∞ëÂãæ?∏‰??∞Â≠∏?üÈõª??), 400

    

    if cmd == "open_url" and not url_val:

        return jsonify(ok=False, error="Ë´ãËº∏?•Ë??ãÂ??ÑÁ∂≤?Ä"), 400
            ...
            if g == 0:
                ...
                if action == "set_volume":
                    ...
                    try:
                        ...
                        text_area_insert(f" ?≥È?Ë®≠ÁÇ∫ {v}%Ôºà‰???Web APIÔº?)
...
# ====== [ANNOUNCE] ?≠Á??¨Â??Ä?ãË? APIÔºàËá™?ïÂ?Ê≠•Â?Á´?announce.htmlÔº?======
...
# ====== [EEW] CWA ?∞È?ËΩâÊé•ÔºàÊ?‰æõÁµ¶ eew.html ‰ΩøÁî®Ôº?======
...
def _parse_cwa_float(val):
    ...
    """?óË©¶Â∞?CWA ?≥Â??ÑÁ?Á∑ØÂ∫¶ÔºèÊ∑±Â∫¶Â?‰∏≤Ë???floatÔºõÂ§±?óÂ??ûÂÇ≥ None??""
...
def fetch_cwa_events(limit=10):
    ...
    """??CWA ?ñÂ??ÄËøëÂú∞?áÔ?È°ØË? + Â∞èÂ??üÔ?ÔºåË??êÁ∞°?ñÂ?Ë°®Áµ¶ /eew/cwa_feed ‰ΩøÁî®??""
        ...
        try:
            ...
            for e in eqs:
                ...
                for area in shaking_areas:
                    ...
                    # CWA format: "AreaIntensity": "4Á¥? or "5Âº?

                    # "CountyName": "?±ËìÆÁ∏?, "TownName": "..."
                ...
                events.append({
                    ...
                    "title": report_text or f"{origin_time} {loc} Ë¶èÊ®°{mag_val}",
                    ...
                    "shaking_areas": area_intensities, # [New] { "?∞Â?Â∏?: "2Á¥?, ... }
...
def fetch_cwa_warnings(limit=10):
    ...
    """??CWA ?ñÂ?Â§©Ê∞£Ë≠¶Áâπ?±Ô?W-C0033-001ÔºâÔ??ûÂÇ≥?óË°®??""
        ...
        for r in records:
            ...
            for loc in locations:
                ...
                for h in hazards:
                    ...
                    # We care about "?∏‰?Âº∑È¢®" (Strong Wind) or "È¢±È¢®" (Typhoon)

                    if phenomena in ["?∏‰?Âº∑È¢®", "È¢±È¢®"]:
...
def eew_cwa_feed():
    ...
    """?ê‰??çÁ´Ø eew.html ?•Ë©¢ CWA ?ÄËøëÂú∞?áÁî®ÔºõUSGS ‰ªçÁî±?çÁ´Ø?¥Êé•?ì„Ä?""
...
CWA_LICENSE_SITE = "" # e.g. "?∫Â?Â∏ÇÂ??óÂ??üÂ?È´ò‰∏≠"
...
def _load_trial_info():
    ...
    try:
        ...
        if diff.days >= TRIAL_DAYS:
            TRIAL_EXPIRED = False # [MOD] Âª∂Èï∑Ë©¶Áî®??(‰∏çÈ?ÂÆöÂ???
...
def _try_load_cwa_license():
    ...
    Local City/County (e.g. '?∫Â?Â∏?) if not set in config.
    """
        ...
        try:
            ...
            if idx1 != -1 and idx2 != -1:
                ...
                if b64_str:
                    ...
                    taiwan_places = [
                        "?∫È?Â∏?, "?∫Â?Â∏?, "?∞Â?Â∏?, "?∞Â?Â∏?, "Ê°ÉÂ?Â∏?, "?∞Á´πÂ∏?, "?∞Á´πÁ∏?,
                        "?óÊ?Á∏?, "?∫‰∏≠Â∏?, "?∞‰∏≠Â∏?, "ÂΩ∞Â?Á∏?, "?óÊ?Á∏?, "?≤Ê?Á∏?, "?âÁæ©Â∏?,
                        "?âÁæ©Á∏?, "?∫Â?Â∏?, "?∞Â?Â∏?, "È´òÈ?Â∏?, "Â±èÊù±Á∏?, "ÂÆúËò≠Á∏?, "?±ËìÆÁ∏?,
                        "?∫Êù±Á∏?, "?∞Êù±Á∏?, "ÊæéÊ?Á∏?, "?ëÈ?Á∏?, "???Á∏?
                    ]
                    ...
                    for am in acc_matches:
                        ...
                        if pwd_match:
                             ...
                             # It often contains "È´ò‰∏≠", "?ã‰∏≠", "Â∞èÂ≠∏" or specific city/town prefix
...
def _cwa_poll_once(silent=False):
    ...
    """?ÆÊ¨°Ëº™Ë©¢?èËºØÔºåËã•?âÊñ∞‰∏îÈ°Ø?óÂú∞?áÂ?Âª?í≠"""
    ...
    if not CWA_API_KEY:
        ...
        CWA_LAST_ERROR = "?™Ë®≠ÂÆ?CWA API Key"
    ...
    try:
        ...
        if ev["id"] != last_id:
            ...
            intensity_str = f"?ÄÂ§ßÈ?Â∫?{ev.get('intensity', '0')}"

            if target_city and local_intensity != "0":

                intensity_str += f"Ôºå{target_city} {local_intensity}"

            

            msg = f"?êÂú∞?áÈÄüÂ†±?ë{ev['time']} {ev['location']} Ë¶èÊ®° {ev['mag']} Ê∑±Â∫¶ {ev['depth']}km {intensity_str}"
            ...
            if not silent:
                ...
                if CWA_BROADCAST_ENABLED:
                    ...
                    def _int_val(s):
                        ...
                        if s in ("5Âº?, "5-"): return 5.0

                        if s in ("5Âº?, "5+"): return 5.5

                        if s in ("6Âº?, "6-"): return 6.0

                        if s in ("6Âº?, "6+"): return 6.5
                ...
                if target_city:
                    ...
                    relevant = [w for w in warnings if w["location"] == target_city and w["phenomena"] in ["?∏‰?Âº∑È¢®", "È¢±È¢®"]]
...
# ====== [4-Relay Web API] ‰ΩøÁî®?®Á? COMÔºàRELAY4_PORTÔºâÊéß??======
...
def relay4_status():
    ...
    """?ûÂÇ≥ 4-Relay ?ÆÂ??Ä?ãÁµ¶ relay4.html ‰ΩøÁî®Ôºå‰∏¶?ê‰??ØÈÅ∏ COM Ê∏ÖÂñÆ??""

    # ?çÁ´Ø?ØÈÄèÈ? /relay4/status?port=COMx ?áÂ??¨Ê¨°?™Â?‰ΩøÁî®??4-Relay COM

    override_port = (request.args.get("port") or "").strip() or None



    # ÊØèÊ¨°?ºÂè´?ΩÈ??∞Ê???4-Relay ?ôÈÅ∏ COMÔºåÊ?‰æõÁµ¶?çÁ´Ø‰∏ãÊ??∏ÂñÆ‰ΩøÁî®
    ...
    # ??Relay ?ÆÂ?‰ΩøÁî®??COMÔºàÁµ¶?çÁ´ØÈ°ØÁ§∫?åÂñÆ Relay Â∑≤Áî®?çÁî®Ôº?

    single_port = (RELAY_INFO.get("port") if "RELAY_INFO" in globals() else None) or RELAY_PORT



    # Ê±∫Â??ô‰?Ê¨°Â??±Áµ¶?çÁ´Ø??4-Relay ‰∏ªË? COM

    if override_port:

        # ?ÅË®±?çÁ´Ø?´Ê??áÂ??ê‰?È°?COM ??4-Relay ‰ΩøÁî®?ÆÊ?
        ...
        # Ê≤íÊ??áÂ?Â∞±Ê≤ø?®Â??¨Á? auto_detect / ?ãÂ?Ë®≠Â??èËºØ

        # [Fix] ÁßªÈô§ get_manual_relay4_port() Ê™¢Êü•ÔºåÁ¢∫‰ø?RELAY4_INFO ?™Ë®≠ÂÆöÊ??ÉÈÄ≤ÂÖ• auto_detect (?ßÈÉ®?ÉË???manual)
...
def relay4_set():
    ...
    """Ë®≠Â??ê‰?Ë∑ØÁπº?ªÂô® ON/OFFÔºöJSON {ch:1-4, on:true/false, port?:COMx}??""
    ...
    try:
        ...
        if ch not in (1, 2, 3, 4):
            ...
            return jsonify(ok=False, error="ch ÂøÖÈ???1~4"), 400
        ...
        # ?êÂ?ÂæåÔ??åÊ≠•?ûÂÇ≥?Ä?∞Á??ãÔ??ÖÂê´?ØÁî® COM Ê∏ÖÂñÆ?áÂñÆ Relay COM
        ...
        with RELAY4_LOCK:
            ...
            # ??[NEW] ?≥Ê? WebSocket Âª?í≠ÔºöË??Ä?âÈ???relay4.html ?ÑÂÆ¢?∂Á´Ø?≥Ê??åÊ≠•
            try:
...
def _try_libretranslate(q: str, source: str, target: str) -> str:
    ...
    raise RuntimeError("‰∏äÊ∏∏??translatedText")
...
def translate_api():
    try:
        ...
        try:
            ...
            return jsonify(ok=False, error=f"?Ä?âÁøªË≠ØÂ??éÂ?Â§±Ê?: {em}"), 502
    except Exception as e:
...
def _validate_timetable(j: dict) -> tuple[bool, str | None]:
    ...
    if not isinstance(j, dict): return False, "payload ÂøÖÈ???JSON ?©‰ª∂"

    items = j.get("items", [])

    if not isinstance(items, list): return False, "items ÂøÖÈ??ØÈô£??

    for i, it in enumerate(items):

        if not isinstance(it, dict): return False, f"items[{i}] ‰∏çÊòØ?©‰ª∂"

        t = (it.get("time") or "").strip()

        if not re.match(r"^\d{2}:\d{2}$", t): return False, f"items[{i}].time ?Ä??HH:MM"

        a = (it.get("action") or "").strip()

        if not a: return False, f"items[{i}].action ‰∏çÂèØÁ©∫ÁôΩ"

        if "date" in it:

            if not re.match(r"^\d{4}-\d{2}-\d{2}$", str(it["date"])): return False, f"items[{i}].date ?Ä??YYYY-MM-DD"

        else:

            d = it.get("dow")

            if d not in (1,2,3,4,5,6,7): return False, f"items[{i}].dow ÂøÖÈ???1..7"
...
def api_timetable_import_holidays():
    ...
    """ÂæûË??øÈô¢‰∫∫‰?Ë°åÊîøÁ∏ΩË? (via CDN) ?ØÂÖ•?∞ÁÅ£?ãÂ??áÊó• (?ÖÂ??≥Ô?‰∏çÂ?Ê™?"""
    ...
    try:
        ...
        # ?ûÂÇ≥?ìÂà∞?ÑË??ôÁµ¶?çÁ´ØÔºåÁî±?çÁ´ØÊ±∫Â??ØÂê¶?à‰Ωµ
...
def _auto_watch_timetable():
    ...
    """ÊØ?30 ÁßíÊ™¢??timetable.json ?ØÂê¶Ë¢´Â??®Êõ¥?∞Ô??•Ê??áËá™?ïÈ?Ëºâ„Ä?""
    ...
    try:
        ...
        if os.path.isfile(TIMETABLE_PATH):
            ...
            if m and _TIMETABLE_MTIME and m > _TIMETABLE_MTIME:
                ...
                text_area_insert("?µÊ∏¨??timetable.json ËÆäÊõ¥ÔºåÂ∑≤?™Â??çË???, "Timetable")
...
def _auto_watch_schedules():
    ...
    """ÊØ?30 ÁßíÊ™¢??schedules.json ?ØÂê¶Ë¢´Â??®Êõ¥?∞Ô??•Ê??áËá™?ïÈ?Ëºâ‰∏¶?∑Êñ∞ UI??""
        ...
        if SCHEDULES_PATH.exists():
            ...
            if m and _SCHEDULES_MTIME and m > _SCHEDULES_MTIME:
                ...
                try:
                    ...
                    ui_safe(text_area_insert, " ?µÊ∏¨??schedules.json ËÆäÊõ¥ÔºåÂ∑≤?™Â??çË???, "Schedules")
...
def api_timetable_enable():
    ...
    try:
        ...
        # ?åÊ≠•ÂæåÁ´Ø GUIÔºàËã•?âÂ???Tk ‰∏ªÊéß?∞Ô?

        def _apply_ui():

            status = " Ë¶èÂ?‰ªªÂ?ÔºöÂ??? if enabled else " Ë¶èÂ?‰ªªÂ?ÔºöÂ???
...
def api_timetable_set():
    ...
    try: text_area_insert(f"Â∑≤Ë?ÂØ´Ë™≤Ë°®Ô?{len(timetable_data.get('items', []))} Á≠ÜÔ?enabled={timetable_enabled}Ôº?)
    except Exception: pass
...
# == [ANCHOR] /schedules API ?áÂ?Á´ØËº™Ë©?==
...
def api_post_schedules():
    ...
    try:
        ...
        # ?ØÊè¥?ÆÁ? dict ?ñÂ?Á≠?listÔºõÊ?ÁµÇÂØ´?•Ê?Ê°àÈÉΩ??list[dict]
        ...
        try:
            ...
            ui_safe(lambda: (refresh_sched_tree(), text_area_insert(" ?™Ë??íÁ?Â∑≤Êõ¥?∞Ô?/schedules ‰∏äÂÇ≥Ôº?, "Web")))
...
def _load_schedules_from_disk():
    ...
    """ËÆÄ??schedules.jsonÔºå‰∏¶?¥Êñ∞‰øÆÊîπ?ÇÈ?Ôºå‰? UI/?çÁ´Ø?åÊ≠•??""
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                text_area_insert(f'?†Ô? /schedules ?íÁ??®ÈåØË™§Ô?{e}')
        ...
        if mode in ("tts", "text"):
            ...
            def _run_tts():
                ...
                try:
                    ...
                    text_area_insert(f"Ë™ûÈü≥?≠ÊîæÔºàÂø´?∑Ô?Ôºö{text}", "Shortcut")
...
# ‰æù‰Ωø?®ËÄÖÈ?Ê±ÇÔ??¥Â??∞Â?ÂæåÁ´ØÔºå‰?ÂΩ±Èüø?¢Ê??üËÉΩ
...
class TaigiTTSClient:
    ...
    def synthesize(self, text: str, voice: str = "normal_f2", model: str = "model6", speed: float = 1.0, user_id: str = ""):
        ...
        Args:
            text (str): Ê¨≤Â??êÁ??∞Ë??áÂ? (Âª∫Ë≠∞‰ΩøÁî®Êº¢Â??ñÊº¢ÁæÖÊ∑∑????
            voice (str): 'normal_m2' (?? ??'normal_f2' (Â•???
            model (str): ?êË®≠ 'model6' (È´òÂ?Ë≥???
            speed (float): Ë™ûÈÄü„Ä?
            user_id (str): ‰ΩøÁî®??ID??
        
        Returns:
            str: ?≥Ê?‰∏ãË? URL (72Â∞èÊ??âÊ?)??
        """
        payload = {
            "text": text,
            "model": model,
        ...
        try:
            # ‰ΩøÁî® _post_with_fallback ‰øùÊ??áÂ?Á≥ªÁµ±‰∏Ä?¥Á?Á∂≤Ë∑ØË´ãÊ??èËºØ (?•Ê??ÄË¶?
            # ‰ΩÜÊ≠§?ïÁ??¨‰Ωø??requests.postÔºåÊ??ëÁõ¥?•‰Ωø??requests.post ?≥ÂèØÔº?
            # ?†ÁÇ∫ _post_with_fallback ?ØË®≠Ë®àÁµ¶Â§öÂÄ?URL ?Ñ„Ä?
            # ?ôË£°?™Ê?‰∏Ä??URLÔºåÁõ¥?•Áî® requests.post??
            # ?∫‰?‰øùÊ??áÂ?Á®ãÂ?Á¢ºÁ??ØË™§?ïÁ?‰∏Ä?¥ÊÄßÔ??ëÂÄëÁ?‰ΩúË™ø?¥„Ä?
            ...
            if response.status_code == 200:
                ...
                raise TaigiTTSException(f"?ÉÊï∏?ØË™§ (400): {response.text}", status=400)
            elif response.status_code == 401:
                raise TaigiTTSException("API Key ?°Ê? (401)", status=401)
            elif response.status_code == 429:
                raise TaigiTTSException("Ë´ãÊ??ªÁ??éÈ? (429)", status=429)
            else:
            ...
            if resp.status_code == 200:
                with open(save_path, 'wb') as f:
                    for chunk in resp.iter_content(chunk_size=8192):
                        if chunk:
                            f.write(chunk)
                # print(f"Ê™îÊ?Â∑≤ÂÑ≤Â≠òËá≥: {save_path}")
            else:
...
def taigi_say():
    ...
    """‰∏ÄÊ¨°Â??êÔ??ØÈÅ∏?å‰∏≠??>?∞Ë?ÁøªË≠Ø?çÔ? TTSÔºå‰∏¶?ØÈÅ∏?áÊòØ?¶Âú®?¨Ê??¥Êé•?≠ÊîæÔºàÂª£?≠Ô???

    - play=True ?ñÊú™?ê‰?ÔºöÂ??êÂ?ÔºåÈÄèÈ? taigi_play_wav_with_fx() Âª?í≠?≠Êîæ

    - play=FalseÔºöÂè™?¢Á?Ê™îÊ??á‰?ËºâÁ∂≤?ÄÔºå‰??üÂ?Âª?í≠ÔºàÁµ¶?çÁ´ØË©¶ËÅΩÔºè‰?ËºâÁî®Ôº?
    ...
    try:
        ...
        # ?™Ê? direction == "zh2nan" ?çÂ?ÁøªË≠ØÔºõÂÖ∂È§òÔ?rawÔºâÂ∞±?¥Êé•??text ??TTS
        ...
        # Ëß?? play ?óÊ?ÔºöÈ?Ë®?TrueÔº?0"/"false" Á≠âË??∫È???
        ...
        # ?àÊ?‰∏¶Â?Ê™îÔ?Ê≤øÁî® /taigi/tts ?èËºØÔº?
        ...
        if play and file_name:
            ...
            def _play():
                ...
                try:
                    ...
                    taigi_play_wav_with_fx(p)  # Â•óÁî®?¥Êé•?≠ÊîæË¶èÊ†ºÔºàRelayÔºãÂ?Â∞?ÁµêÊ?Ôº?
...
def udp_listener():
    ...
    text_area_insert(f" UDP ??ÅΩ?üÂ?Ôº?.0.0.0:{PORT}", src="UDP")
    ...
    while True:
        ...
        try:
            ...
            text_area_insert(f"UDP ?ØË™§Ôºö{e}")
...
def start_ngrok(force=False):
    ...
    if not exe:
        ...
        text_area_insert("[ngrok] ?™Êâæ??ngrokÔºåÂèØÂ∞?USE_NGROK=False ?ñÂ?Ë£?ngrok", src="System"); return
    ...
    try:
        ...
        try:
            ...
            ui_safe(ngrok_status_label.config, text=f" ngrokÔºö{STATE.get('ngrok_url') or '?™Â?Âæ?URL'}")
        ...
        try: ui_safe(ngrok_status_label.config, text=f" ngrok ?üÂ?Â§±Ê?Ôºö{e}")
...
def stop_ngrok():
    ...
    try: ui_safe(ngrok_status_label.config, text=" ngrokÔºöÂ∑≤?ãÂ??úÊ≠¢", fg=THEME["fg_sub"])
...
THEME = {
    ...
    "bg_root": "#F3F4F6",       # ?îÂ??∞ÁôΩ?åÊôØ

    "bg_header": "#1E293B",     # Ê∑±Ë???Header

    "fg_header": "#F8FAFC",     # Header ?áÂ?

    "bg_card": "#FFFFFF",       # ?°Á??ΩÂ?

    "bg_status": "#FFFFFF",     # ?Ä?ãÂ??ΩÂ?

    "fg_text": "#1F2937",       # Ê∑±ÁÅ∞?áÂ?

    "fg_sub": "#6B7280",        # Ê∑∫ÁÅ∞Ë™™Ê??áÂ?

    "accent": "#3B82F6",        # ?ê‰∫Æ??

    "border": "#E5E7EB",        # ?äÊ?

    "btn_bg": "#FFFFFF",        # ?âÈ??åÊôØ
...
# ====================================================
# == [ANCHOR] HEADLESS MODE (Render / ?°Ê??¢Áí∞Â¢? ==
# ====================================================
if not _HAS_TKINTER:
    # ??tkinterÔºöÁõ¥?•Â??ïÊ??âÂ??∞Ê??ô‰∏¶?ªÂ?Ôºà‰?Âª∫Ê? GUIÔº?
    print("[HEADLESS] No tkinter ??starting web-only mode")

    # ÂÆöÁæ©‰∏Ä?ãÁ©∫??root ?ø‰ª£?©Ô??§‰? after() ‰πãÂ?Âπæ‰??Ω‰??ÄË¶ÅÔ?
    class _FakeRoot:
        def after(self, *a, **kw): pass
        def mainloop(self): pass
        def destroy(self): pass
    ...
    # ?üÂ? Flask Web ServerÔºàÂ??∞ÈòªÂ°ûÔ?
...
# ====================================================
# == [ANCHOR] GUI MODE (Windows Ê°åÈù¢?∞Â?) ==
# ====================================================
...
# [ANCHOR] Ë®≠Â? AppUserModelID ??ICON

# ?ôÊòØËÆ?Windows Â∑•‰??óËÉΩÈ°ØÁ§∫Ê≠?¢∫?ñÁ§∫?ÑÈ??µÔ?ÂøÖÈ??®Ê≠§Ë®≠Â?

try:

    myappid = 'mycompany.relaybell.client.1228v1' # ‰ªªÊ?Â≠ó‰∏≤ÔºåÂîØ‰∏Ä?≥ÂèØ
...
root.title("OmniSignal | Â∞àÊ•≠?®Â?Âª?í≠Á≥ªÁµ±")
...
def _lighten_hex(color: str, delta: int = 18) -> str:
    ...
    """Á∞°ÂñÆ‰∫??óË™ø?¥Â??≠ÈÄ≤‰??≤Á¢ºÔºõÂ§±?óÂ??ûÂÇ≥?üËâ≤??""
...
tk.Label(header, text="UDP Âª?í≠Ë™ûÈü≥?•Êî∂‰º∫Ê???, fg=THEME["fg_header"], bg=THEME["bg_header"],
...
def open_timetable_tab():
    ...
    except Exception: messagebox.showinfo("Ë™≤Ë°®", "Ë™≤Ë°®?ÜÈ?Â∞öÊú™Âª∫Á?")





btn_cancel = tk.Button(header_tools, text="??Âº∑Âà∂?ñÊ?",

                       command=lambda: handle_msg("CancelALL", ("?¨Ê?",)),
...
btn_clear = tk.Button(header_tools, text="Ê∏ÖÈô§Ë®äÊÅØ",
...
ip_label = tk.Label(header_right, text=f"?¨Ê? IPÔºö{local_ip}", fg="#94A3B8", bg=THEME["bg_header"], font=THEME["font_s"])
...
status_label = tk.Label(status_row, text=" Ë™ûÈü≥?üÁî®‰∏?, fg="#059669", font=THEME["font_bold"], bg=THEME["bg_card"])

status_label.pack(side="left", padx=0, pady=5)



ngrok_status_label = tk.Label(status_row, text=" ngrokÔºöÊú™?üÂ?", font=THEME["font_s"], bg=THEME["bg_card"], fg=THEME["fg_sub"])
...
# ?∞Â??ãÂ??ãÈ??âÈ? (Assign to variables only)

ngrok_start_btn = tk.Button(ngrok_btn_frame, text="?üÂ? ngrok", bg="#D1FAE5", relief="flat", font=THEME["font_s"], command=lambda: start_ngrok(force=True))

ngrok_start_btn.pack(side="left", padx=(0, 4))



ngrok_stop_btn = tk.Button(ngrok_btn_frame, text="?úÊ≠¢ ngrok", bg="#FEE2E2", relief="flat", font=THEME["font_s"], command=stop_ngrok)
...
def update_ngrok_ui_state(is_running):
    ...
    if is_running:
        ...
        ngrok_start_btn.config(state="disabled", text="?üÂ?‰∏?..", bg="#E5E7EB")

        ngrok_stop_btn.config(state="normal", text="?úÊ≠¢ ngrok", bg="#FEE2E2")

    else:

        ngrok_start_btn.config(state="normal", text="?üÂ? ngrok", bg="#D1FAE5")

        ngrok_stop_btn.config(state="disabled", text="Â∑≤Â?Ê≠?, bg="#E5E7EB")
...
copy_btn = tk.Button(ngrok_btn_frame, text="Ë§áË£Ω ngrok URL", bg="#F1F5F9", relief="flat", font=THEME["font_s"])

copy_btn.pack(side="left", padx=(0,6))



# Ôºà‰??ôË???label ‰ª•Á∂≠?ÅÂ?ËßÄÔºå‰?‰∏çÂ??®Â?ÂΩàÂá∫ QRÔº?
...
# === ?∞Â?ÔºöÈ??üÂ?Á´?+ ?çÁ´Ø QRÔºàÁç®Á´ãË?Á™óÔ?===

# === ?∞Â?ÔºöÈ??üÂ?Á´?+ ?çÁ´Ø QRÔºàÁç®Á´ãË?Á™óÔ?===

front_btn = tk.Button(ngrok_btn_frame, text="?ãÂ??çÁ´ØÔºàQR Ë¶ñÁ?Ôº?, bg="#F1F5F9", relief="flat",
...
# ‰øùÁ? labelÔºà‰?‰ΩøÁî® toggleÔºâÔ?Á∂≠Ê?Â§ñË?
...
def copy_ngrok_url_and_qr_popup():
    ...
    if not url:
        ...
        ui_safe(ngrok_status_label.config, text=" ngrokÔºöÂ??™Â?Âæ?URL", fg="#b33"); return

    try:

        root.clipboard_clear(); root.clipboard_append(url)

        ui_safe(ngrok_status_label.config, text=f" ngrokÔºö{url}", fg="#074")

    except Exception as e:

        ui_safe(ngrok_status_label.config, text=f" Ë§áË£ΩÂ§±Ê?Ôºö{e}", fg="#b33")

    show_qr_popup("ngrok ?¨Á∂≤Á∂≤Â? QR", url)
...
tk.Button(status_row, text=" ?çÂ?‰º∫Ê???, command=restart_app, bg="#F1F5F9", relief="flat",

          font=THEME["font_s"]).pack(side="right", padx=(6,8), pady=4)



tk.Label(status_row, text=" Á≠âÂ??•Êî∂Ë®äÊÅØ‰∏?..", font=THEME["font_s"], bg=THEME["bg_card"], fg=THEME["fg_sub"]).pack(side="right", padx=12)
...
tab_main = tk.Frame(notebook, bg=THEME["bg_root"]); notebook.add(tab_main, text=" ‰∏ªÊéß??)



# ---- Tab X: 4?ëRelay ----

tab_relay4 = tk.Frame(notebook, bg=THEME["bg_root"]); notebook.add(tab_relay4, text=" 4?ëRelay")



# Status label for 4?ëRelay

relay4_status_var = tk.StringVar(value="PortÔºö‚ÄîÔ?CmdÔºö‚ÄîÔ?ResultÔºö‚ÄîÔ?ErrÔºö‚Ä?)
...
tk.Label(relay4_ctrl_frame, text="‰ΩøÁî® COMÔº?, font=THEME["font_s"], bg=THEME["bg_card"]).grid(row=0, column=0, sticky="w")
...
def apply_manual_relay4_port():
    ...
    """?ãÂ??áÂ? 4-Relay COM ?†‰∏¶ÂØ´ÂÖ• relay4_port.txt??""
    ...
    try:
        ...
        if not port:
            ...
            messagebox.showinfo("4-Relay", "Ë´ãÂ??∏Ê? COM ??é•??)
        ...
        messagebox.showinfo("4-Relay", f"Â∑≤Ë®≠ÂÆ?4-Relay ‰ΩøÁî® {port}\n‰∏ãÊ¨°?üÂ?‰πüÊ?Â•óÁî®")

    except Exception as e:

        messagebox.showerror("4-Relay", f"ÂØ´ÂÖ• relay4_port.txt Â§±Ê?Ôºö\n{e}")
...
def refresh_relay4_ui():
    ...
    try:
        ...
        relay4_status_var.set(
            ...
            f"PortÔºö{RELAY4_INFO.get('port') or RELAY4_PORT or '??'}  "

            f"CmdÔºö{RELAY4_INFO.get('last_cmd') or '??'}  "

            f"ResultÔºö{RELAY4_INFO.get('last_result') or '??'}  "

            f"ErrÔºö{RELAY4_INFO.get('last_error') or '??'}"
...
tk.Button(relay4_ctrl_frame, text="Â•óÁî®", command=apply_manual_relay4_port, bg="#F3F4F6", relief="flat", font=THEME["font_s"]).grid(row=0, column=2, padx=4, pady=2)
...
tk.Button(relay4_ctrl_frame, text="?çÊñ∞?ÉÊ?", command=relay4_rescan, bg="#F1F5F9", relief="flat", font=THEME["font_s"]).grid(row=0, column=3, padx=4, pady=2)



# ?öÈ??ßÂà∂ÔºàÂ∑•Ê•≠È¢®?íÂ?ÔºöÁ??ãÔ??âÈ?Ôº?
...
for ch in range(1,5):
    ...
    sv = tk.StringVar(value=f"CH{ch}Ôºö‚Ä?)
...
tab_eew = tk.Frame(notebook, bg=THEME["bg_root"]); notebook.add(tab_eew, text=" ?∞È? (EEW)")



# Status

eew_status_var = tk.StringVar(value="ËºâÂÖ•‰∏?..")
...
tk.Label(eew_ctrl_frame, text="Ë®≠Â?Ôº?, font=THEME["font_bold"], bg=THEME["bg_card"]).pack(side="left", padx=8, pady=8)
...
chk_eew_en = tk.Checkbutton(eew_ctrl_frame, text="?üÁî®?™Â?Ëº™Ë©¢", variable=eew_enable_var, bg=THEME["bg_card"], font=THEME["font_main"])
...
chk_eew_bc = tk.Checkbutton(eew_ctrl_frame, text="?üÁî®Ë≠¶Â†±Âª?í≠", variable=eew_broadcast_var, bg=THEME["bg_card"], font=THEME["font_main"])

chk_eew_bc.pack(side="left", padx=8)



tk.Label(eew_ctrl_frame, text="?ìÈ?(s)Ôº?, bg=THEME["bg_card"]).pack(side="left", padx=2)
...
tk.Label(eew_ctrl_frame, text="KeyÔº?, bg=THEME["bg_card"]).pack(side="left", padx=2)
...
tk.Label(eew_ctrl_frame, text="?éÂ?Ôº?, bg=THEME["bg_card"]).pack(side="left", padx=2)

eew_city_var = tk.StringVar(value="")

_TW_CITIES = ["(?™Â??µÊ∏¨)", "?∫È?Â∏?, "?∞Â?Â∏?, "?∞Â?Â∏?, "Ê°ÉÂ?Â∏?, "?∞Á´πÂ∏?, "?∞Á´πÁ∏?, "?óÊ?Á∏?, "?∞‰∏≠Â∏?, "ÂΩ∞Â?Á∏?, "?óÊ?Á∏?, "?≤Ê?Á∏?, "?âÁæ©Â∏?, "?âÁæ©Á∏?, "?∞Â?Â∏?, "È´òÈ?Â∏?, "Â±èÊù±Á∏?, "ÂÆúËò≠Á∏?, "?±ËìÆÁ∏?, "?∞Êù±Á∏?, "ÊæéÊ?Á∏?, "?ëÈ?Á∏?, "???Á∏?]
...
tk.Label(eew_ctrl_frame, text=" (?ôÁ©∫?™Â??µÊ∏¨) ", font=THEME["font_s"], bg=THEME["bg_card"], fg="#64748B").pack(side="left")
...
tk.Label(eew_ctrl_frame2, text="?áÂ∫¶?ÄÊ™ªÔ?", bg=THEME["bg_card"]).pack(side="left", padx=2)

eew_threshold_var = tk.StringVar(value="3")

# We can use a Combobox for common values

eew_threshold_combo = ttk.Combobox(eew_ctrl_frame2, textvariable=eew_threshold_var, values=["1", "2", "3", "4", "5Âº?, "5Âº?, "6Âº?, "6Âº?, "7"], width=5)
...
def _apply_cwa_ui():
    ...
    try:
        ...
        messagebox.showinfo("EEW", f"Â∑≤Êõ¥?∞Ë®≠ÂÆö\n?üÁî®Ôºö{CWA_ENABLED}\n?éÂ?Ôºö{CWA_LOCAL_CITY or '(?™Â?)'}\n?ÄÊ™ªÔ?{CWA_INTENSITY_THRESHOLD}\nKeyÔºö{CWA_API_KEY[:4]}***")
...
tk.Button(eew_ctrl_frame, text="Â•óÁî®", command=_apply_cwa_ui, bg="#3B82F6", fg="white", relief="flat").pack(side="left", padx=12)
...
tk.Button(eew_act_frame, text="[?™] Ê∏¨Ë©¶Ë≠¶Â†±?≥Ê?", command=_test_cwa_alarm, bg="#F59E0B", padx=8, relief="flat").pack(side="left", padx=(0,8))

tk.Button(eew_act_frame, text="[?ª] Á´ãÂç≥Ê™¢Êü• CWA", command=lambda: threading.Thread(target=_cwa_poll_once, args=(True,), daemon=True).start(), bg="#10B981", relief="flat", padx=8).pack(side="left")
...
def _update_eew_ui_loop():
    ...
    try:
        ...
        if CWA_LOCAL_CITY:
            ...
            if eew_city_var.get() != "(?™Â??µÊ∏¨)":

                eew_city_var.set("(?™Â??µÊ∏¨)")
        ...
        def _update_auto_city_task():
            ...
            try:
                ...
                if city:
                    ...
                    eew_auto_city_var.set(f" [Â∑≤ÂÅµÊ∏¨‰∏¶Â•óÁî®: {city}]")

                    _save_cwa_conf()

                else:

                    eew_auto_city_var.set(" [?µÊ∏¨‰∏çÂà∞?∑È??éÂ?]")

            except Exception as e:

                eew_auto_city_var.set(f" [?µÊ∏¨Â§±Ê?: {e}]")

        

        # Only update if currently set to "(?™Â??µÊ∏¨)"

        if not CWA_LOCAL_CITY and eew_city_var.get() == "(?™Â??µÊ∏¨)":
        ...
        rev_mapping = {"5-": "5Âº?, "5+": "5Âº?, "6-": "6Âº?, "6+": "6Âº?}
        ...
        status = f"Ëº™Ë©¢Ôºö{'?üÁî®' if CWA_ENABLED else '?úÁî®'}ÔΩúÂª£?≠Ô?{'?üÁî®' if CWA_BROADCAST_ENABLED else '?úÈü≥'}ÔΩúÂ?Â∏ÇÔ?{CWA_LOCAL_CITY or '(?™Â??µÊ∏¨)'}ÔΩúÈ?Ê™ªÔ?{thresh_display}ÔΩúÊ? {CWA_POLL_SEC} Áß?
        ...
        if CWA_LAST_ERROR:
            ...
            txt = f"[{d.get('time')}] {d.get('location')} Ë¶èÊ®°{d.get('mag')} Ê∑±Â∫¶{d.get('depth')}km\n{d.get('title')}"
...
eew_city_var.set(CWA_LOCAL_CITY if CWA_LOCAL_CITY else "(?™Â??µÊ∏¨)")
...
tk.Label(setting_panel, text=" Ë®≠Â?Ë™ûË?Ôº?, font=THEME["font_bold"], bg=THEME["bg_card"]).grid(row=0, column=0, padx=(16,0), pady=16, sticky="w")

lang_label_var = tk.StringVar(value="?™Â??µÊ∏¨ (Auto)")
...
tk.Label(setting_panel, text="Ô∏??ßÂà•Ôº?, font=THEME["font_bold"], bg=THEME["bg_card"]).grid(row=0, column=2, padx=(24,0), pady=16, sticky="w")

gender_label_var = tk.StringVar(value="Â•≥ËÅ≤")
...
tk.Label(setting_panel, text=" Ë™ûÈÄüÔ?", font=THEME["font_bold"], bg=THEME["bg_card"]).grid(row=0, column=4, padx=(24,0), pady=16, sticky="w")
...
rate_scale = tk.Scale(rate_frame, from_=-50, to=50, orient="horizontal", length=126,
                      ...
                      command=lambda v: handle_msg(f"SetRate:{int(float(v))}%", ("?¨Ê?",)))
...
tk.Label(rate_frame, text="Âø?, font=THEME["font_s"], fg="#666", bg=THEME["bg_card"]).pack(side="left", padx=(2,0))
...
tk.Button(quick_panel, text="Â•≥ËÅ≤", command=lambda: handle_msg("Girl", ("?¨Ê?",)), bg="#F3F4F6", relief="flat", font=THEME["font_s"]).pack(side="left", padx=2)

tk.Button(quick_panel, text="?∑ËÅ≤", command=lambda: handle_msg("Boy", ("?¨Ê?",)),  bg="#F3F4F6", relief="flat", font=THEME["font_s"]).pack(side="left", padx=2)

tk.Button(quick_panel, text="?úÈü≥", command=lambda: handle_msg("Mute", ("?¨Ê?",)),  bg="#FEE2E2", relief="flat", font=THEME["font_s"]).pack(side="left", padx=2)

tk.Button(quick_panel, text="Ëß?ô§?úÈü≥", command=lambda: handle_msg("Unmute", ("?¨Ê?",)), bg="#D1FAE5", relief="flat", font=THEME["font_s"]).pack(side="left", padx=2)
...
volume_label = tk.Label(volume_panel, text=f"?≥È?Ôºö{STATE['volume']}%", font=THEME["font_bold"], bg=THEME["bg_card"])
...
tk.Button(volume_panel, text="Ôº?", command=lambda: handle_msg("VolDown", ("?¨Ê?",)), bg="#F3F4F6", relief="flat", font=THEME["font_s"]).pack(side="left", padx=4)

tk.Button(volume_panel, text="Ôº?", command=lambda: handle_msg("VolUp", ("?¨Ê?",)),   bg="#F3F4F6", relief="flat", font=THEME["font_s"]).pack(side="left", padx=4)
...
tk.Label(trans_preview_frame, text="?? ÁøªË≠Ø?êË¶ΩÔº?, font=THEME["font_bold"], bg="#EFF6FF", fg="#1E40AF").pack(side="top", anchor="w", padx=16, pady=(8,2))
...
tk.Button(trans_btn_row, text="?? ?≠ÊîæÊ≠§ÁøªË≠?, command=_gui_play_trans, bg="#3B82F6", fg="white", relief="flat", font=THEME["font_s"]).pack(side="right", padx=4)
tk.Button(trans_btn_row, text="???úÈ??êË¶Ω", command=lambda: trans_preview_frame.pack_forget(), bg="#94A3B8", fg="white", relief="flat", font=THEME["font_s"]).pack(side="right", padx=4)
...
tk.Label(play_panel, text="Ô∏??áÂ??≠Â†±Ôº?, font=THEME["font_bold"], bg=THEME["bg_card"]).grid(row=0, column=0, padx=(16,4), pady=6, sticky="w")
...
def _submit_tts():
    ...
    if txt:
        ...
        threading.Thread(target=handle_msg, args=(txt, ("?¨Ê?",)), daemon=True).start()

tk.Button(play_panel, text="?≠Â†±", command=_submit_tts, bg="#D1FAE5", relief="flat", font=THEME["font_s"]).grid(row=0, column=2, padx=6, pady=6)

def _gui_translate():
    q = speak_entry.get().strip()
    if not q:
        messagebox.showinfo("?∫ÊÖßÁøªË≠Ø", "Ë´ãÂ?Ëº∏ÂÖ•Ë¶ÅÁøªË≠ØÁ??áÂ?")
        return
    ...
    trans_preview_text.insert("1.0", "??ÁøªË≠Ø‰∏?..")
    trans_preview_text.config(state="disabled")
    trans_preview_text.config(state="disabled")
    ...
    def _task():
        try:
            ...
            if out:
                ...
                ui_safe(lambda: messagebox.showerror("ÁøªË≠ØÂ§±Ê?", "?°Ê??≤Â?ÁøªË≠ØÁµêÊ?"))
                ui_safe(lambda: trans_preview_frame.pack_forget())
        except Exception as e:
            ui_safe(lambda: messagebox.showerror("ÁøªË≠Ø?ØË™§", str(e)))
            ui_safe(lambda: trans_preview_frame.pack_forget())
...
tk.Button(play_panel, text="?∫ÊÖßÁøªË≠Ø", command=_gui_translate, bg="#DBEAFE", relief="flat", font=THEME["font_s"]).grid(row=0, column=3, padx=6, pady=6)



# ?∞Ë??≠Â†±

tk.Label(play_panel_extra, text="Ô∏??∞Ë??≠Â†±Ôº?, font=THEME["font_bold"], bg=THEME["bg_card"]).grid(row=0, column=0, padx=(16,4), pady=6, sticky="w")
...
def _submit_taigi():
    ...
    if txt:
        ...
        threading.Thread(target=handle_msg, args=(f"PlayTaigi:{txt}", ("?¨Ê?",)), daemon=True).start()

tk.Button(play_panel_extra, text="?≠Âè∞Ë™?, command=_submit_taigi, bg="#FDE68A", relief="flat", font=THEME["font_s"]).grid(row=0, column=2, padx=6, pady=6)
...
tk.Label(play_panel_extra, text="Ô∏??®Ëû¢ÂπïË??ØÔ?", font=THEME["font_bold"], bg=THEME["bg_card"]).grid(row=1, column=0, padx=(16,4), pady=6, sticky="w")
...
tk.Button(play_panel_extra, text="È°ØÁ§∫ÔºãÊí≠??, command=lambda: handle_msg(f"ShowMsg:{fs_entry.get().strip()}", ("?¨Ê?",)), bg="#FEF3C7", relief="flat", font=THEME["font_s"]).grid(row=1, column=2, padx=6, pady=6)

tk.Button(play_panel_extra, text="?™È°ØÁ§?,   command=lambda: handle_msg(f"SilentMsg:{fs_entry.get().strip()}", ("?¨Ê?",)), bg="#FEF3C7", relief="flat", font=THEME["font_s"]).grid(row=1, column=3, padx=6, pady=6)
...
tk.Label(play_panel, text=" YouTube ??MP3 ???Ôº?, font=THEME["font_bold"], bg=THEME["bg_card"]).grid(row=3, column=0, padx=(16,4), pady=6, sticky="w")
...
def _submit_media():
    ...
    if ("youtube.com/watch" in url) or ("youtu.be/" in url) or ("/shorts/" in url):
        ...
        threading.Thread(target=handle_msg, args=(url, ("?¨Ê?",)), daemon=True).start()

    else:

        threading.Thread(target=handle_msg, args=(f"PlayMP3:{url}", ("?¨Ê?",)), daemon=True).start()

tk.Button(play_panel, text="?≠Êîæ???", command=_submit_media, bg="#DBEAFE", relief="flat", font=THEME["font_s"]).grid(row=3, column=2, padx=6, pady=12)



def _choose_mp3_local():

    p = filedialog.askopenfilename(title="?∏Ê? MP3", filetypes=[("MP3", "*.mp3")])

    if p:

        threading.Thread(target=handle_msg, args=(f"PlayMP3:{p}", ("?¨Ê?",)), daemon=True).start()

tk.Button(play_panel, text="?≠Êîæ?¨Ê? MP3", command=_choose_mp3_local, bg="#DBEAFE", relief="flat", font=THEME["font_s"]).grid(row=3, column=3, padx=6, pady=12)
...
tk.Label(student_ctrl_panel, text=" Â≠∏Á?Á´ØÊéß?∂Ô?", font=THEME["font_bold"], bg=THEME["bg_card"]).pack(side="left", padx=(16,4), pady=10)

tk.Label(student_ctrl_panel, text="?ÆÊ? ID (Á©??®ÈÉ®)Ôº?, font=THEME["font_s"], bg=THEME["bg_card"]).pack(side="left", padx=2)
...
def _send_student_cmd_ui(cmd_key, args=""):
    ...
    if target == "ALL":
        ...
        text_area_insert(f"Â∑≤Âª£?≠Ê?‰ª?({cmd_key}) ?≥Ê??âÂ≠∏??, "Student")
        ...
        if success:
             text_area_insert(f"Â∑≤Áôº?ÅÊ?‰ª?({cmd_key}) ??{target}", "Student")
        else:
             text_area_insert(f"?ºÈÄÅÊ?‰ª?({cmd_key}) ??{target} Â§±Ê?", "Student")

tk.Button(student_ctrl_panel, text="?øÈà¥", command=lambda: _send_student_cmd_ui("Ring"), bg="#FDE68A", relief="flat", font=THEME["font_s"]).pack(side="left", padx=4)
tk.Button(student_ctrl_panel, text="?ãÂ?Á∂≤È?", command=lambda: _send_student_cmd_ui("OpenURL", "https://google.com"), bg="#DBEAFE", relief="flat", font=THEME["font_s"]).pack(side="left", padx=4)
tk.Button(student_ctrl_panel, text="?çÊñ∞?üÂ?", command=lambda: _send_student_cmd_ui("Reboot"), bg="#FEE2E2", relief="flat", font=THEME["font_s"]).pack(side="left", padx=4)
tk.Button(student_ctrl_panel, text="?úÊ?", command=lambda: _send_student_cmd_ui("Shutdown"), bg="#FEE2E2", relief="flat", font=THEME["font_s"]).pack(side="left", padx=4)
...
playing_label = tk.Label(progress_row, text="??Á≠âÂ??≠Êîæ‰∏≠‚Ä?, bg=THEME["bg_root"], font=THEME["font_s"], fg=THEME["fg_sub"])
...
tk.Label(
    ...
    text=" USB ÁπºÈõª?®Ô?CH340/CH341Ôº?,
...
relay_status_var = tk.StringVar(value="PortÔºö‚ÄîÔ?CmdÔºö‚ÄîÔ?ResultÔºö‚ÄîÔ?ErrÔºö‚Ä?)
...
def apply_manual_relay_port():
    ...
    if not port:
        ...
        messagebox.showwarning("USB ÁπºÈõª??, "Ë´ãÂ??∏Ê?Ë¶Å‰Ωø?®Á? COM ?†„Ä?)
    ...
    try:
        ...
        messagebox.showinfo("USB ÁπºÈõª??, f"Â∑≤Ê?ÂÆöÁπº?ªÂô®‰ΩøÁî® {port}\n‰∏ãÊ¨°?üÂ?‰πüÊ??™Â?Â•óÁî®??)

    except Exception as e:

        messagebox.showerror("USB ÁπºÈõª??, f"ÂØ´ÂÖ• relay_port.txt Â§±Ê?Ôºö\n{e}")
...
def refresh_relay_ui():
    ...
    try:
        ...
        relay_status_var.set(
            ...
            f"PortÔºö{RELAY_INFO.get('port') or '??}ÔΩ?

            f"CmdÔºö{RELAY_INFO.get('last_cmd') or '??}ÔΩ?

            f"ResultÔºö{RELAY_INFO.get('last_result') or '??}ÔΩ?

            f"ErrÔºö{RELAY_INFO.get('last_error') or '??}"
...
tab_tt = tk.Frame(notebook, bg=THEME["bg_root"]); notebook.add(tab_tt, text=" Ë¶èÂ?‰ªªÂ?")
...
timetable_status_var = tk.StringVar(value=" Ë¶èÂ?‰ªªÂ?Ôºö‚Ä?)
...
timetable_next_var = tk.StringVar(value=" ‰∏ã‰?Ê¨°Ô???)
...
timetable_path_var = tk.StringVar(value=f"Ë∑ØÂ?Ôºö{TIMETABLE_PATH}")
...
def _tt_enable():
    ...
    handle_msg("ScheduleEnable", ("?¨Ê?",))

def _tt_disable():

    handle_msg("ScheduleDisable", ("?¨Ê?",))

def _tt_reload():

    handle_msg("ScheduleReload", ("?¨Ê?",))

    _load_timetable_from_disk(); update_next_label()



tk.Button(tt_top, text="?üÁî®",  command=_tt_enable,  bg="#D1FAE5", relief="flat", font=THEME["font_s"]).pack(side="right", padx=4)

tk.Button(tt_top, text="?úÁî®",  command=_tt_disable,  bg="#FEE2E2", relief="flat", font=THEME["font_s"]).pack(side="right", padx=4)

tk.Button(tt_top, text="?çÊñ∞ËºâÂÖ•", command=_tt_reload, bg="#F1F5F9", relief="flat", font=THEME["font_s"]).pack(side="right", padx=(4, 16))
...
tt_tree.heading("when", text="?ÇÈ?ÔºàDOW/?•Ê?Ôº?)

tt_tree.heading("action", text="?ï‰?")

tt_tree.heading("label", text="Ê®ôÁ±§")
...
def _tt_play_selected():
    ...
    if not sel:
        ...
        messagebox.showinfo("Ë™≤Ë°®", "Ë´ãÂ??∏Ê?‰∏ÄÁ≠ÜÂ??≠Êîæ")
...
tk.Button(tt_bottom, text="?≠Êîæ?Ä??, command=_tt_play_selected, bg="#DBEAFE", relief="flat", font=THEME["font_s"]).pack(side="left", padx=4)

tk.Button(tt_bottom, text="?çÊñ∞?¥Á?", command=refresh_timetable_tree, bg="#F1F5F9", relief="flat", font=THEME["font_s"]).pack(side="left", padx=4)
...
tab_files = tk.Frame(notebook, bg=THEME["bg_root"]); notebook.add(tab_files, text=" Ê™îÊ?/‰∏äÂÇ≥")
...
tk.Label(files_top, text=" ‰∏äÂÇ≥?ÆÈ?Ôº?, font=THEME["font_bold"], bg=THEME["bg_card"]).pack(side="left", padx=(16,0), pady=12)
...
def _open_folder(path):
    ...
    try:
        ...
        messagebox.showerror("?ãÂ?Ë≥áÊ?Â§?, f"?°Ê??ãÂ?Ôºö{e}")



tk.Button(files_top, text="?ãÂ?Ë≥áÊ?Â§?, command=lambda:_open_folder(UPLOAD_DIR), bg="#F1F5F9", relief="flat", font=THEME["font_s"]).pack(side="right", padx=(4, 16))
...
files_tree.heading("name", text="Ê™îÂ?")

files_tree.heading("size", text="Â§ßÂ?")

files_tree.heading("mtime", text="‰øÆÊîπ?ÇÈ?")
...
def refresh_files():
    ...
    try:
        ...
        messagebox.showerror("Ê™îÊ??óË°®", str(e))
...
def _play_selected_file():
    ...
    if not sel:
        ...
        messagebox.showinfo("?≠Êîæ", "Ë´ãÂ??∏Ê?‰∏ÄÈ¶?MP3")
    ...
    threading.Thread(target=handle_msg, args=(f"PlayMP3:{path}", ("?¨Ê?",)), daemon=True).start()
...
def _delete_selected_file():
    ...
    if not sel:
        ...
        messagebox.showinfo("?™Èô§", "Ë´ãÂ??∏Ê?‰∏ÄÈ¶?MP3")
    ...
    if not messagebox.askyesno("?™Èô§", f"Á¢∫Â??™Èô§Ê™îÊ?Ôºö{name}Ôº?):
    ...
    try:
        ...
        messagebox.showerror("?™Èô§Â§±Ê?", str(e))



def _upload_from_local():

    ps = filedialog.askopenfilenames(title="?∏Ê? MP3 ‰∏äÂÇ≥", filetypes=[("MP3", "*.mp3")])
    ...
    for p in ps or []:
        ...
        try:
            ...
            messagebox.showerror("‰∏äÂÇ≥Â§±Ê?", f"{os.path.basename(p)}Ôºö{e}")
...
tk.Button(files_bottom, text="?çÊñ∞?¥Á?", command=refresh_files, bg="#F1F5F9", relief="flat", font=THEME["font_s"]).pack(side="left", padx=4)

tk.Button(files_bottom, text="?≠Êîæ?Ä??, command=_play_selected_file, bg="#DBEAFE", relief="flat", font=THEME["font_s"]).pack(side="left", padx=4)

tk.Button(files_bottom, text="?™Èô§?Ä??, command=_delete_selected_file, bg="#FEE2E2", relief="flat", font=THEME["font_s"]).pack(side="left", padx=4)

tk.Button(files_bottom, text="ÂæûÊú¨Ê©üË?Ë£ΩÂà∞ uploads", command=_upload_from_local, bg="#F1F5F9", relief="flat", font=THEME["font_s"]).pack(side="left", padx=4)
...
tab_sched = tk.Frame(notebook, bg=THEME["bg_root"]); notebook.add(tab_sched, text=" ?™Ë??íÁ?")
...
sched_status_var = tk.StringVar(value="‰∏ã‰?Ê¨°Ô???)

tk.Label(sched_top, text="?™Ë??íÁ?Ôº?schedules.jsonÔº?, font=THEME["font_bold"], bg=THEME["bg_card"]).pack(side="left", padx=(16,12), pady=12)
...
tk.Button(sched_top, text="?ãÂ?Ë≥áÊ?Â§?, command=_open_sched_file, bg="#F1F5F9", relief="flat", font=THEME["font_s"]).pack(side="right", padx=(4, 16))
...
def refresh_sched_tree():
    ...
    if st.get("has_next"):
        ...
        sched_status_var.set(f"‰∏ã‰?Ê¨°Ô?{nxt.get('at')}ÔΩú{nxt.get('title') or nxt.get('payload')}")

    else:

        sched_status_var.set("‰∏ã‰?Ê¨°Ô???)
...
def _sched_play_selected():
    ...
    if not sel:
        ...
        messagebox.showinfo("?íÁ?", "Ë´ãÂ??∏Ê?‰∏ÄÁ≠ÜÂ?Ëß∏Áôº")
    ...
    if typ == "sendmp3":
        ...
        handle_msg(f"PlayMP3:{payload}", ("?¨Ê?",))

    else:

        handle_msg(str(payload), ("?¨Ê?",))



sched_bottom = tk.Frame(tab_sched, bg=THEME["bg_root"]); sched_bottom.pack(fill="x", padx=0, pady=(0,16))

tk.Button(sched_bottom, text="?çÊñ∞?¥Á?", command=refresh_sched_tree, bg="#F1F5F9", relief="flat", font=THEME["font_s"]).pack(side="left", padx=4)

tk.Button(sched_bottom, text="Á´ãÂç≥Ëß∏Áôº?Ä??, command=_sched_play_selected, bg="#DBEAFE", relief="flat", font=THEME["font_s"]).pack(side="left", padx=4)



# ---- Tab 5: Ë®≠Â? ----
tab_settings = tk.Frame(notebook, bg=THEME["bg_root"]); notebook.add(tab_settings, text=" Ë®≠Â?")
...
tk.Label(settings_frame, text="Á≥ªÁµ±Ë®≠Â?", font=THEME["font_bold"], bg=THEME["bg_card"]).pack(anchor="w", padx=16, pady=(12, 6))
...
chk_ai = tk.Checkbutton(settings_frame, text="?üÁî® AI Âª?í≠Á®øÁ???(?ÄÂÆâË? Ollama)", variable=ai_gen_var, bg=THEME["bg_card"], font=THEME["font_main"])
chk_ai.pack(anchor="w", padx=16, pady=(0, 12))

tk.Label(settings_frame, text="Ë™™Ê?ÔºöËã•?úÁî®Ê≠§Â??ΩÔ?Â∞á‰??ÄÂÆâË? Ollama Ê®°Â?ÔºåÈÅ©?à‰??çÁΩÆ?ªËÖ¶‰ΩøÁî®??, font=THEME["font_s"], fg=THEME["fg_sub"], bg=THEME["bg_card"]).pack(anchor="w", padx=36, pady=(0, 16))
...
def run_web_server():
    ...
    print(">>> Á≥ªÁµ±?üÂ??êÂ? (?àÊú¨ V1214.01-FINAL) <<<")
    print(f">>> Á∂≤È?‰ªãÈù¢Ôºöhttp://localhost:{HTTP_PORT} <<<")
    print("[*] " + "="*50 + "\n")
    
    if DISABLE_WEB:
        text_area_insert(" DISABLE_WEB=TrueÔºåÊú™?üÂ? Flask??, "Web")
        return
    host = "0.0.0.0"; port = HTTP_PORT

    try:
        text_area_insert(f" Flask ‰º∫Ê??®Â???(?ØÊè¥ WebSocket)Ôºöhttp://{host}:{port}", "Web")
        app.run(host=host, port=port, threaded=True, use_reloader=False)
    except Exception as e:
        text_area_insert(f"??Web ‰º∫Ê??®Â??ïÂ§±?óÔ?{e}", "Web")
...
try:
    ...
    # [NEW] Auto-Repair Edge TTS  (?ûÈòªÂ°ûÁ? ??‰∏çÂü∑Ë°?pip install ‰ª•ÈÅø?çÈ??âÊ??°‰?)
    def _check_and_repair_edge_tts():
        try:
            print(">>> [Self-Check] Ê≠?ú®Ê™¢Êü• Edge TTS ÂºïÊ????...")
            ...
            if ok:
                print(">>> [Self-Check] Edge TTS ?ã‰?Ê≠?∏∏ (OK)")
                STATE["edge_tts_status"] = "OK"
            else:
                raise Exception("Output file empty or missing")

        except Exception as e:
            print(f">>> [Self-Check] Edge TTS ?∞Â∏∏ ({e})")
            STATE["edge_tts_status"] = f"Error: {str(e)[:30]}"
            # ‰∏çËá™??pip install ???øÂ?Á®ãÂ??úÈ??ÇÂ? subprocess ?ªÂ??åÁÑ°Ê≥ïÁ??≥Â?Ê≠?
            # ?•È?Ë¶Å‰øÆÂæ©Ë??ãÂ??∑Ë?Ôºöpip install --upgrade edge-tts
            print(">>> [Self-Check] ?•È?‰øÆÂæ© Edge TTS Ë´ãÊ??ïÂü∑Ë°? pip install --upgrade edge-tts")
    ...
    if not DISABLE_UDP:
        ...
        text_area_insert(" UDP ?•Êî∂Â∑≤Â??®Ô?DISABLE_UDP=TrueÔº?, "UDP")
                ...
                try:
                    ...
                    if is_active:
                        if not self.triggered:
                            ...
                            threading.Thread(target=handle_msg, args=("ShowMsg:?êÁÅ´?ΩË≠¶?±„ÄëÂÅµÊ∏¨Âà∞?´ÁÅΩË®äË?ÔºåÈÄ≤Ë?Á∑äÊÄ•Âª£?≠Ô?", ("System", "Fire")), daemon=True).start()
...
def keep_alive_pinger():
    ...
    if not url:
        print("?†Ô? [Keep-Alive] No RENDER_EXTERNAL_URL found, pinger aborted.")
        return
    ...
    while True:
        try:
            ...
            print(f"?†Ô? [Keep-Alive] Self-ping failed: {e}")
